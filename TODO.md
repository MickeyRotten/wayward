...
---
[x] Add Personality & Drive field(s) to the PC character, as major context for the action suggestions.
...
---
[x] Automatic regenerate if the LLM returns an error or safety trigger, e.g. 3 retries (configurable).

Done: new **`OpenRouterSettings.auto_retry_count`** (default 2, range 0-5, Config → Agents & Tools; 0 = off/manual RETRY as before). On an OpenRouter error or a safety-filter block (both raised as `RuntimeError`), the turn silently regenerates up to that many extra times before the error surfaces. Implemented as **per-model-call retry** — two helpers in `openrouter.py` (`agent_turn_with_retry` for the agentic narrator + Editor, `stream_with_retry` for the legacy text path) re-invoke a fresh stream on failure; because the transcript is only appended after a *successful* result, a retry never re-runs tools that already committed (the key safety property, since the agentic loop commits tools per round). A partial attempt emits a `discard` (client clears the half-written text) then a `retry` event → the chat shows "Retrying (n/N)". Applies to the Narrator (agentic + legacy paths) and the Editor. Verified: retry-helper unit tests (retry-then-succeed with discard/retry events, exhaustion raises after N+1 attempts, retries=0 = single attempt, legacy text path), settings migration + round-trip + clamp to 5, and a Playwright test where a mocked turn emitting a retry event recovers cleanly (final rendered, discarded partial gone, no error). tsc + build clean.

---
[x] Research LiteLLM and add NVIDIA NIM (deepseek-ai/deepseek-v4-pro by default) as an alternative provider.

Done (commit 143e601) — researched LiteLLM, found it **incompatible** with this project and did NOT use it: LiteLLM requires pydantic v2 + heavy native deps (Rust tokenizers, tiktoken, grpcio, polars, uvloop) that have no Android wheels, so embedding it would break the pydantic-v1-pinned APK; its only real benefit (unifying 100+ providers) is moot when the target providers are already OpenAI-compatible. NVIDIA NIM's hosted API (`https://integrate.api.nvidia.com/v1`, `nvapi-…` bearer key) is exactly the OpenAI-compatible streaming/tool-call shape `openrouter.py` already speaks, so it needs no adapter.

Built a **lightweight direct provider abstraction** instead (zero new deps — only httpx, already bundled):
- **Server** — `OpenRouterSettings` gains `llm_provider` (`openrouter`|`nvidia_nim`|`custom`) + per-provider write-only creds/model (`nim_api_key`/`nim_model_id`, `custom_base_url`/`custom_api_key`/`custom_model_id`), with 6 additive app-db migrations. One seam — `provider_endpoint(settings) -> (base_url, api_key, main_model)` — resolves the active provider; `base_url` is threaded through every LLM function (`chat_completion_stream`/`_agent_turn`/`_text`/`fetch_models`, default `OPENROUTER_BASE`) and every call site (narrator, planner/Editor, Chronicler, action-suggester, summarizer, vision, legacy text path, `/models`, and the turn guards). Two provider quirks handled centrally: `_apply_sampling(openai_strict=…)` drops OpenRouter-only params (`min_p`/`top_k`/`repetition_penalty`) off-OpenRouter (NIM rejects them), and `fetch_models` maps NIM/custom's plain `{data:[{id}]}` to the normalized shape (tool-capable, no context length) vs OpenRouter's rich metadata. Per-agent model overrides stay model ids on the active provider. Keys are write-only (`nimApiKeySet`/`customApiKeySet` mirror `apiKeySet`); export/import carry the non-secret provider fields and preserve existing keys.
- **Client** — Config → AI & Model gains a **Provider dropdown** (OpenRouter / NVIDIA NIM / Custom); switching persists immediately + refetches the model list, prefills NIM's default `deepseek-ai/deepseek-v4-pro`, and conditionally shows that provider's masked key field (+ an API Base URL field for Custom) with a where-to-get-a-key hint. The model picker/`/models` already serve the active provider's list.
- **Verified** — unit (`provider_endpoint`, `is_openrouter`, `openai_strict` sampling, NIM/OpenRouter model mapping); integration (real streaming + a tool-call round + strict sampling driven against a local OpenAI-compatible SSE stub via a resolved custom base_url); config round-trip on a real app boot under **pydantic v1** (migration adds the columns, NIM PUT/GET round-trips, keys stay write-only and survive an update that omits them); and Playwright (provider dropdown + conditional key/base-URL fields + NIM prefill). Client tsc + vite build clean; **APK unaffected** (no new pip pins).

---
[x] Chat frequently errors with "Generation Error / name 'streamed' is not defined" regardless of provider.
...
---
[x] R1 (logic, high): Lorebook keyword matching should scan recent history, not just the newest player message. `build_prompt` calls `match_entries(player_message, …)` — so a keyword the NARRATOR just introduced never injects its lore (narrator names "Kal-Toth", player replies "I ask about the ruin" → no injection), and short player replies ("Yes", "I follow her") inject nothing at all. Match against the last N turns of history (narrator + player, active variants), with a configurable scan depth on LorebookConfig (default ~3 turns). Consider also treating the entry title as an implicit keyword so entries work without manual keyword setup.

  Done: keyword matching now scans a window — the new player message PLUS the last `scan_depth` turns of history (both roles, newest messages; the history passed to build_prompt is already active-variant/narrator-thread only). `LorebookConfig.scan_depth` (default 3, clamp 0–20; 0 = old behavior) with an additive campaign-db migration, threaded through GET/PUT `/lore/config` (`scanDepth`) and shared types; Config → World → Lorebook Injection gains a "Keyword scan depth" field. Entry **titles are now implicit keywords** (case-insensitive, same substring rule), so entries match without manual keyword setup; disabled entries still never match. Verified: unit tests (title match, narrator-introduced keyword injects at depth 3 and NOT at depth 0), API round-trip incl. clamp + partial-update preservation, and a real migration back-fill test (column dropped → `_run_scope_migrations` restores it); client tsc + build clean.

---
[x] R2 (perf/UX, high): Move threshold summarisation out of the turn. When the context threshold trips, `_maybe_summarize_and_build` runs a full summariser LLM call BEFORE narration starts — the player just sees a long stall on exactly the turns that are already slowest (long adventures). Run summarisation post-turn in the background instead (like the Chronicler): after a turn completes, check the threshold and compress then, so the NEXT turn's prompt is already trimmed. Keep an in-turn fallback only for the degenerate case where the prompt genuinely won't fit the context even after trimming.

  Done: `_maybe_summarize_and_build` no longer summarises — it builds the prompt (build_prompt already trims oldest history to the budget, so the turn always fits) and returns a `needs_summary` flag when over the threshold. Both stream drivers (agentic + legacy, so sends/swipes/regenerates all count) schedule `_summarize_in_background()` AFTER the narration is saved and the `done` event has gone out: a fire-and-forget task on its own session that compresses the oldest unsummarised turns via the existing `pick_messages_to_summarize`/`generate_summary` (optional summary model honored), guarded by a lock so concurrent turns can't double-compress, errors only logged (retries naturally on a later turn). No explicit degenerate-case fallback needed — trimming covers it, exactly as it already did between thresholds. The now-dead `summarized` SSE event was removed server-side (the client's handler tolerates its absence). Verified: stubbed-LLM integration test (8 seeded turns → background pass compresses 6 messages up to turn 3, StorySummary committed, `/journal` reflects it); client tsc + build clean.

---
[x] R3 (compat): Reasoning-model support. The stream parsers in openrouter.py read only `delta.content` and ignore reasoning deltas (`delta.reasoning`/`reasoning_content`). With a reasoning model (e.g. DeepSeek R1 — plausible on NIM, whose default is a DeepSeek id) the UI sits on a silent THINKING indicator for the whole reasoning phase, and reasoning tokens spend `max_tokens` so the narration can arrive truncated or empty with no explanation. Surface reasoning deltas as live status (word count or a collapsible "thinking" block in the streaming window), make sure an all-reasoning/empty-content result gives a useful error, and consider exposing OpenRouter's `reasoning` effort param in Config → AI & Model.

  Done: both stream parsers now read reasoning deltas (`delta.reasoning`, OpenRouter's normalized field, AND `delta.reasoning_content`, the DeepSeek-style variant). `chat_completion_agent_turn` yields them as `reasoning` events; `chat_completion_stream` gained a `yield_events` mode (default off — the summariser/vision callers keep the original string contract) that yields content/reasoning/usage dicts, used by the legacy narrator driver (where tool-less reasoning models actually land) and the continue driver; `stream_with_retry` forwards both modes. The chat indicator shows the thinking phase live — "REASONING · ~N WORDS" with the elapsed timer (`chatStore.reasoningChars`, cleared when narration starts/on retry). An all-reasoning/empty-narration result (budget spent thinking) now surfaces a clear error ("raise Max Response Tokens or lower Reasoning Effort") instead of saving an empty beat — guarded on the agentic, legacy, and continue paths. New **Reasoning Effort** setting (Config → AI & Model → Sampling: provider default / low / medium / high; `OpenRouterSettings.reasoning_effort`, additive migration, clamped server-side, in adventure export/import) sent as OpenRouter's `reasoning.effort` on the narrator's calls — never sent to strict (NIM/custom) providers, which may reject it. Verified: mock-transport SSE tests through the REAL parsers (both reasoning field variants, default-mode contract unchanged, retry-wrapper forwarding), `_apply_sampling` strict/effort/bogus-value tests, settings round-trip incl. bogus → ''; client tsc + build + vitest clean.

---
[x] R4 (infra): Committed test suite + CI. Every verification so far has been a throwaway script — nothing is committed (no tests/ directory), so regressions like the `streamed` NameError only surface in play. Codify the load-bearing invariants into a real pytest suite (prompt budget/trimming, spotlight regexes, instance grant/equip/reversal, Chronicler reversal incl. `_prev` restore, inline-options parsing, template application, provider endpoint resolution) plus client unit tests for the pure libs (narration.ts segmenter, weather.ts, backdrops.ts, sortEntries.ts), and run both in CI on every push/PR — the workflow currently only builds the APK.

  Done: **server** — `server/tests/` (pytest, 50 tests, <2s): lore injector (keywords/titles/permanent/disabled/positions), prompt builder (budget trimming, first-message reservation, post-history placement, roster fields, R1 scan window, equipment rendering), spotlight (word-boundary names, dialogue-vs-mention attribution, group address, field-skill relevance, custom rule), action options (first-person backstop, inline `<<<OPTIONS>>>` parse incl. truncated-JSON salvage, rule normalisation), provider endpoint resolution + `openai_strict` sampling, summariser helpers, and TestClient integration tests (template boot seed, lore-config round-trip/clamp, story export, narrator grant/equip through the real tools with exact-state reversal, Chronicler update `_prev` snapshot/restore, background summary with a stubbed LLM). Safety keystone: new `WAYWARD_DATA_DIR` env override on `database.py` — conftest points it at a temp dir before any server import, so tests can NEVER touch a real server/data. `server/requirements-dev.txt` carries pytest. **Client** — vitest (dedicated `vitest.config.ts`, node env, no react/tailwind plugins; `npm test`), 20 tests over the pure libs: narration segmenter (dialogue blocks, NPC fallback, last-quote interleaved-attribution rule, trailing-tag split, blockquote/divider, newline robustness), weather mapping (snowstorm>storm, sand→haze), backdrop picker (time vocabulary, default fallback), sortEntries (incl. the missing-field blank-panel regression). **CI** — new `.github/workflows/test.yml` runs both suites (+ client tsc) on every push/PR touching server/client/shared. Verified: 50 pytest + 20 vitest green locally; client tsc + build clean with tests present.

---
[x] R5 (UX): A true Continue. The current Continue pill sends "I wait and let the scene unfold." as a NEW player turn. Add a real continuation that EXTENDS the last narration: re-send the prompt with the assistant message last plus a continue nudge, and append the result to the same message/variant (no new turn). This is also the rescue for narration clipped by max_tokens_response — right now a clipped beat just ends mid-sentence with no recovery.

  Done: new `POST /chat/continue` builds the normal prompt (history up to the latest turn + that turn's player message), appends the active narration variant as the last assistant message plus a CONTINUE nudge (same scene/tense, no repetition, no greeting, no options block), streams prose-only (no tools, no action protocol — auto-retry included), and splices the result onto the SAME ChatMessage: a beat that ended mid-sentence continues with a space, a complete one starts a new paragraph. No new turn, no reversible effects — swipe/regenerate/delete semantics unchanged; a stray inline-options block is stripped defensively. The Continue pill now calls it (`chatStore.continueNarration`), falling back to the old "I wait…" turn when there's no narration yet; post-continue the Chronicler/suggester re-run as usual. Verified: 3 new pytest integration tests with a stubbed stream (extend-in-place with paragraph separator, mid-sentence splice with a space, clean 400 on an empty thread) — which also caught a real bug: the request session's open transaction held the adventure DB lock against the stream's save session (fixed by committing before streaming, same as the turn route).

---
[x] R6 (observability): Real token/cost accounting. The provider's `usage` block is never requested or read — the context meter is a chars/4 estimate. Request usage on the streaming calls, record real prompt/completion tokens (and OpenRouter's reported cost) per message, and surface them in the prompt-log modal + a small per-turn meta line. Also lets us calibrate the chars/4 trim margin against reality instead of the blanket 10% safety haircut.

  Done: usage accounting is requested on every OpenRouter stream (`usage: {include: true}` via `_apply_sampling`; never sent to strict NIM/custom providers, whose responses may still be estimated) and read from the final chunk by both parsers. The narrator agent sums usage across every tool round of a turn; the totals — `prompt_tokens`, `completion_tokens`, `gen_cost` — are stored on the assistant `ChatMessage` (3 additive adventure migrations) and returned as `promptTokens`/`completionTokens`/`cost`. A Continue folds its spend into the same message's numbers. Surfaced as a dim per-message meta label in the message's actions bar — "12,345 → 512 tok · $0.0042" with an explanatory tooltip — and the context meter's `done` value now prefers the provider's real total over the chars/4 estimate (which remains the fallback); real usage is also logged to the terminal per turn. (The prompt-log modal shows the prompt before the call runs, so usage lives on the message/meter instead.) Verified: mock-transport tests assert usage reaches the stream/agent results, DB columns round-trip through `GET /chat/messages`, and the strict-provider test proves `usage` is never sent off-OpenRouter; client tsc + build + vitest clean.

---
[x] R7 (feature/UX): Backdrop manager. `server/backdrops/` is populated by hand and isn't committed — and on the APK there is no way to add backdrops at all (the folder lives inside app storage). Add upload/list/delete endpoints and a Config → Appearance manager (thumbnail grid, upload, delete, filename hint showing how location/time matching works). Consider bundling one or two default arts in the repo so fresh clones/the APK aren't plain, and including backdrops in the campaign export zip.

  Done: `POST /api/backdrops/upload` (png/jpg/webp only, filename sanitized but its words kept — they're what the scene matcher scores) and `DELETE /api/backdrops/{file}` (traversal-guarded like the GET). Config → Appearance gains a **Backdrops** manager: thumbnail grid with per-file delete, an "+ ADD BACKDROP" file picker, and a hint teaching the `city_day.png` naming scheme; `lib/backdrops.ts` gained `invalidateBackdrops()` so the chat's cached list refreshes after changes. This is also the only way to add scene art on the APK. (Bundled default art + backdrops-in-export-zip left out for now — art files in the repo are a licensing/size call.) Verified: pytest upload→list→serve→delete round-trip, bad-extension rejection, traversal guard; client tsc + build clean.

---
[x] R8 (feature): Export the adventure as a readable story. A markdown/text download of the narrator thread — active variants only, dialogue formatted, day/location headers from the declared scene state, no planner/tool noise — from the Saves tab. Cheap to build and makes finished adventures shareable/archivable.

  Done: new `GET /adventure/story-export` renders the active adventure as Markdown — a `# Campaign — Adventure` title from the json sidecars, the First Message as the opening beat, then each turn with the player's action as `**<PC name>:** *action*` and the narration passed through as-is (it's already markdown-ish: bold/italics, "> " inscriptions, "* * *" dividers). `## Day N — Location` headers are emitted whenever a narrator message declares a scene change (carry-forward, same rule as the chat banner). Planner-thread messages are excluded and only each turn's active variant (highest — the client's default view) is included; served as a `text/markdown` attachment named after the adventure. Client: the ACTIVE adventure card in the Saves tab gains an "EXPORT STORY" button (window.open, same pattern as the campaign zip export). Verified: endpoint smoke test over a seeded thread (title/headers/player-line formatting, swiped variant excluded, planner message excluded); client tsc + build clean.
...
---
[x] R12 (safety): Automatic local backups. The SQLite files under server/data ARE the user's worlds and saves; one bad migration or corruption loses a campaign, and the only protection is manual zip export. Cheap insurance: copy campaign.db/adventure.db into a rotating backups/ folder (keep last N) on campaign switch and on app boot before migrations run, with a restore path surfaced in the Saves tab.

  Done: `storage.snapshot_campaign(cid)` writes a self-contained campaign zip — the SAME format as the manual EXPORT (the zip builder was extracted from the export route into `storage.build_campaign_zip` and is now shared) — into `DATA_DIR/backups/`, called at boot for a pre-existing scope BEFORE migrations run (fresh/legacy creations skip it) and on every campaign switch before the target's migrations. Throttled to one snapshot per campaign per 6h, rotated to the newest 10, and never raises (a failed backup must not block boot). **Restore is deliberately non-destructive**: `POST /backups/{file}/restore` feeds the zip through the shared campaign-import path (extracted as `_import_campaign_bytes`), so a snapshot always comes back as a NEW name-deduped campaign — live data is never overwritten. The Saves tab gains a **Backups** section (list + age + RESTORE with confirm; restoring switches to the restored campaign). Verified: pytest — snapshot content/throttle, list endpoint, restore creates a new campaign (count +1, fresh id), rotation caps at 10, traversal guard; the campaign EXPORT round-trips through the shared builder (existing export test path unchanged).

---
[x] R13 (feature, small): Alternate openings. Allow several First Messages per campaign (like SillyTavern's alternate greetings): at turn 0 the swipe arrows cycle them, and the chosen one anchors the adventure. `first_message_options` already made the opening beat structured — this is the narration-side sibling, and templates could ship variety.

  Done: **Authoring** — `NarratorConfig.first_message_alternates` (campaign-scoped JSON) holds additional openings beside the primary `first_message`. Edited in the First Message Inspector card (Edit Mode) as an "Alternate Openings" list of multiline `ExpandableTextarea`s (view mode lists them read-only), served/saved via `/narrator` (`firstMessageAlternates`, blanks trimmed like `firstMessageOptions`), carried by templates (`firstMessageAlternates`) and campaign export/import. The Editor also gets a `set_first_message_alternates` tool. **Turn-0 swipe** — the chat opening block now renders `[first_message, *alternates]` with ‹ / › arrows + an "OPENING n / N" counter when there's more than one greeting and the adventure hasn't started (`chatStore.openingIndex`, clamped so a stale cross-adventure index can't overflow). **Anchoring** — the greeting showing when the player takes their first turn rides along on the turn request (`ChatTurnRequest.opening`); on the opening turn the server locks it into a new adventure-scoped `StorySummary.opening_message`, which then overrides the campaign primary in both the chat display (`GET /chat/opening`, fetched in `fetchHistory`) and the narrator prompt (`build_prompt(first_message_override=…)`, threaded through `_maybe_summarize_and_build`) and the Export Story markdown. Deleting the whole narrator thread (clear chat, or delete-and-after back to empty) releases the anchor so the arrows return. Additive migrations for both new columns; `set_first_message`/primary path untouched. Verified: 69 pytest (2 new: build_prompt override fallback + the anchor/opening-endpoint/reset round-trip), 20 vitest, client tsc + vite build clean.

  Follow-up (R13b — per-opening options + Scenario-tab cards): options are now **opening-specific**. `first_message_alternates` changed shape from `list[str]` to `list[{message, options}]` (a shared `normalize_openings` in `ai/scenario.py` coerces the legacy R13 bare-string shape, so no migration needed; a typed `OpeningAlt` schema, `firstMessageAlternates` read/write through it, and the `set_first_message_alternates` Editor tool now takes `{message, options}` items). Each opening's message + options fold into a single ordered `openings` array (`buildOpenings` = `[{first_message, first_message_options}, ...alternates]`); at turn 0 the action panel shows the **currently-swiped opening's own** options (`ChatScene` picks `openings[openIdx].options`). **UX** — the Scenario tab now has an **"Opening Messages"** section header with one card per opening (primary + each alternate, EMPTY badge for blanks) and a **+ NEW** button (Edit Mode) that appends an opening and selects it; each card opens in the Inspector to edit *that* opening's narration + its own Opening Options, with a "✕ REMOVE THIS OPENING" for alternates. The old single-card "Alternate Openings" list editor is gone. Selection ids: primary stays `firstMessage`, alternates use `opening:<index>` (`openingSelId`/`openingIndexOf` in `scenarioFields.ts`). Concurrent debounced message/options saves for one alternate merge onto the freshest store array (they share one field). Verified: 70 pytest (round-trip test updated to objects + a `normalize_openings` unit test), 20 vitest, client tsc + vite build clean.

---
[x] R14 (maintainability, low): Split the two megafiles. routes.py is 3,343 lines and ChatScene.tsx 2,254 — every change pays a navigation tax and merge risk. Split routes.py into domain routers (chat, campaigns/adventures, characters/party, items/inventory, lore/scenario, settings, tts) under server/api/, and carve ChatScene.tsx into Scene/ subcomponents (banner, action panel, message list, composer, indicators). Pure refactor, no behavior change.

  Done (commit 2cd8772): **Server** — `server/api/routes.py` (3,667 lines by then) is now a 42-line aggregator that stitches 12 domain routers under the same `/api` prefix: `campaigns.py` (adventures, campaigns, backups, adventure export/import/reset + story export), `characters.py` (PC, party members, character library incl. portraits/voices), `backdrops.py`, `lore.py` (scenario + lorebook), `narrator.py` (config, journal, narrator voice), `settings.py` (LLM settings + `/models` proxy), `tts.py`, `items.py` (catalog, inventory, equip/unequip), `tasks.py`, `chat.py` (messages/events, turn/swipe/regenerate/continue, the SSE streaming drivers, prompt log, background summary), `worldbuild.py` (Chronicler proposals + action suggestions), `planner.py` (the Editor turn + queued deletes) — with the cross-domain helpers (`_active_ids`, `_pc_to_response`/`_pm_to_response`, `_item_to_dict`/`_list_inventory_dicts`, chat-image storage) in `server/api/common.py`. `main.py`'s `from server.api.routes import router` is untouched. Verified the route table is IDENTICAL to the old monolith — all 102 method+path pairs diffed via each version's OpenAPI schema — and the full pytest suite passes (the two tests that monkeypatched `routes.*` internals now target `chat`/`campaigns`/`backdrops`). **Client** — `ChatScene.tsx` (2,286 lines by then) is now a ~700-line orchestrator over new `Scene/` modules: `SceneBanner.tsx` (location banner + Play/Edit toggle + time icon), `ActionPanel.tsx` (numbered options + reroll), `Composer.tsx` (fixed quick actions, Tools menu, image attach, the auto-growing input, SEND/STOP — its purely-local state moved in with it), `MessageBubble.tsx` (the memo'd bubble + ActionsBar + ChangeNotices + inline EditArea + CopyButton), `StreamingWindow.tsx`, `Narration.tsx` (formatNarration/entity chips/SegmentedNarration/DialogueBlock/SceneHeader/Portrait), `EditorActionsFeed.tsx`, `EventToast.tsx`, `SearchBar.tsx`, `PromptLogModal.tsx`, `Indicators.tsx` (Elapsed + ThinkingIndicator), and `chatDerived.ts` (buildVisibleMessages/getVariantCounts/computeSceneHeaders). The render invariants held: StreamingWindow remains the sole per-chunk subscriber, and MessageBubble keeps its callback-tolerant comparator with every derived prop still useMemo'd in ChatScene. Pure refactor — no route, prompt, or UI behavior change. Verified: 64 pytest + 20 vitest green, client tsc + vite build clean.

---
[x] R15 (UX, mobile): Edit/Play mode toggle always accessible. On the APK the only mode toggle is the Play button in the chat banner, and the mobile shell shows one view at a time — so from Home/Items/Lore, or with the entry you want to edit already open in the Inspector slide-over, you have to navigate back to the Chat tab, toggle, then navigate back again.
  ...
  Done (one commit with R16 + R17): the mobile More sheet gains an "EDIT MODE" row (pencil icon + ON/OFF state chip, separated by a top border from the tab list) that flips the mode and closes the sheet — reachable from every view; and the Inspector header's EDITING/VIEW badge is now a button that toggles Edit Mode in place (hover styling + "Switch to…" tooltips), with the open entry staying selected thanks to the existing selection-preserving mode flip. The chat-banner Play button is unchanged. Verified: client tsc + vite build clean.

---
[x] R16 (UX, small): Rename the "Home" tab to "Party" — rail tab label + icon tooltip and the mobile nav bar label (the view shows the PC + party roster, so the name should say so).

  Done (same commit): the tab's label in IconRail's shared TABS list is now "Party" (the mobile nav and tooltip reuse it), and the house icon was swapped for a two-person party icon to match. The internal TabId stays 'home' (no churn in uiStore/App routing). Also updated the Voice & Audio blurb's "(Home tab)" reference. Verified: client tsc + vite build clean.

---
[x] R17 (UX): Party Member sheet fields. Personality, Likes, and Dislikes are single-row `Field` inputs on the member sheet (PartyMemberEditor.tsx) — make them large multi-line text areas (the shared ExpandableTextarea, like Description/Field Skill). Also add an "Other" free-text field to basicInfo for anything that doesn't fit the structured fields. Wiring for `other`: extend the `_BASIC_KEYS` whitelist in server/db/characters.py (the drive field was silently dropped by it once already), surface it in the member sheet (view + edit), include it in the narrator's party roster block (prompt_builder.py), and expose it to the Editor's create_member/update_member tools.

  Done (same commit): Personality/Likes/Dislikes on the member sheet are now multi-line TextAreas (the shared ExpandableTextarea wrapper — corner-expand modal included), keeping their teaching placeholders. New "Other" field end to end: `other` added to BasicInfo (shared/types/models.ts), to the `_BASIC_KEYS` whitelist (server/db/characters.py — additive, character.json just gains a key), to the member sheet in edit (TextArea with placeholder) and view (shown only when set), to the narrator's PARTY ROSTER block (prompt_builder.py, injected as "Other: …" when non-empty), and to the Editor's create_member/update_member tool schemas + handlers (planner.py). Verified: `_clean_basic_info` round-trips `other`; a build_prompt smoke test shows "Other: …" in the roster block; client tsc + vite build clean.

---
[x] R18: In Lore, let's rename the current World category into Locations. Let's add a new category (highest order, should have a high impact), called Pillars. These will be rules of the world / universe, etc. Place it after Scenario, but before Locations.

Ensure Editor has access to this, too.

  Done: **"World" → "Locations"** as a pure relabel — the internal `cat` id stays `'world'` (like R16 kept the 'home' TabId) so no data migration is needed and the locked Scenario entry (a `cat='world'` row) is untouched. The globe icon was swapped for a map-pin. **New "Pillars" category** (`cat='pillars'`) added end to end, tab-ordered right after Scenario and before Locations: `LoreCategory` (shared/types/models.ts), the LorePanel sub-tabs, the Settings injection-config list, the Inspector category dropdown + badge colors, and a new classical-columns CategoryIcon. **High impact** = Pillars are *always injected* into the narrator prompt regardless of keywords (treated like `permanent` in `lore_injector.match_entries`, so world rules are always in context — disabled ones still opt out) and sort **first** via injection order 0 (defaults bumped to pillars 0 / world 10 / characters 20 / … across models.py, schemas.py, seed.py, templates.py, campaigns.py; existing campaigns fall back to order 0 / position 'top' with no migration, and persist on next Settings save). **Editor + Chronicler access:** `pillars` added to the shared `LORE_CATS` set (feeds both the Editor's `create_lore`/`update_lore` `cat` enum and the Chronicler's), a shared `LORE_CAT_ORDER` drives both world-state listings (Pillars listed first), and per-category guidance in both agents now describes Pillars as foundational world/universe rules to reserve for load-bearing facts. Verified: 66 pytest green (2 new pillars always-inject tests), 20 vitest green, client tsc + vite build clean.
---

[x] R19: Perform a comprehensive expert evaluation of the Config's usability and UX and create a suggestion to improve it's user experience.

  **Evaluation.** Config is feature-complete and well-annotated but architecturally confused about *saving* and *scope*. Findings:
  - 🔴 **C1 (data loss):** the panel mirrored ~30 store values into local `useState` and re-synced them in one effect keyed on `settings.maxContextTokens`/`settings.provider`. Picking a model (which auto-saves + sets `maxContextTokens`) or switching provider re-fired that effect and reset every *unsaved* local field — so "drag Temperature → pick a model" silently discarded the temperature edit.
  - 🔴 **C2 (two save models, unlabeled):** provider/model/campaign/lorebook-injection/appearance/backdrops/voice/Option-Rules-reset persisted instantly, while sampling/narrator-text/agents/tts/vision/party-size/Option-Rule-text needed the bottom SAVE. Nothing signalled which was which (the Option Rules block edits-need-SAVE but its adjacent RESET applied immediately).
  - 🟠 **H1:** leaving the Config tab unmounted the panel and dropped staged edits with no guard. **H4:** `saveAll` had no `try/catch`, so a failed save showed nothing. **H2:** one long small-type scroll, no nav, non-sticky SAVE with more content *below* it. **H3:** no indication that a setting is app-global vs per-campaign vs per-device.
  - 🟡 Reset-to-defaults was inconsistent (staged for most, immediate for Appearance) and incomplete; "Show all models" silently governs every picker; sampling is an undifferentiated wall with Reasoning Effort misfiled; data management is split across Campaign export / Adventure export / the Saves tab.
  - Strengths kept: collapsible sections, thorough inline help, per-section reset, provider abstraction, live model list, font-size live preview.

  **Implemented (the recommended first slice — resolves every Critical/High finding).** Config is now **fully auto-save, per field**: a new optimistic `patchLocal` on [settingsStore](client/src/state/settingsStore.ts)/[narratorStore](client/src/state/narratorStore.ts) updates the store instantly for a responsive UI while a **debounced flush** (400 ms; immediate for selects/checkboxes/model/provider) PUTs the current store — coalescing rapid edits into one request. The store→local mirror and the two clobbering effects are gone (kills **C1**), and every control reads straight from the store (setter *names* were preserved so the large JSX + the per-section Reset handlers were untouched — resets now apply immediately, fixing the staged/immediate inconsistency). The global **SAVE** button is removed and replaced by a **sticky status header** — "Saving… / All changes saved / Save failed ↻" with a manual retry — that also surfaces save errors (**H4**) and never scrolls away (**H2**); API keys save on blur. The sticky header carries a **jump-nav** (Campaign · AI · Agents · World · Voice · Look) that scrolls to and force-opens a section (section open-state lifted to the panel). Every section/sub-section now shows a **scope chip** — GLOBAL / CAMPAIGN / DEVICE (with a one-line legend) — placed at the sub-section level where a section mixes scopes (Agents), so the label is always accurate (**H3**). Verified: client tsc + vite build clean, 20 vitest green. (Deferred, noted for later: P3 task-based reorg, P4 sampling ergonomics, P5 unifying the split data-management surfaces.)
---

[ ] R20: Perform a comprehensive game design evaluation of Wayward, and create suggestions to improve its player experience and the delivery of the ultimate AI-driven text adventure engine.

  **Plan — suggested actions (prioritized).** Wayward's narration loop is strong (agentic narrator, deterministic spotlight, choices-first action panel, Chronicler world-building, journal recap, dice). The gaps are the things that make a *game* rather than a great chat: growth, stakes that persist, reactive world state, and onboarding. Suggested actions, grounded in what exists:

  1. **Persistent stakes / status track (highest value).** Today a `skill_check` rolls a d20 but the *outcome* is narration-only — nothing carries. Add an adventure-scoped, lightweight **conditions/resources** layer the narrator reads and writes via tools (`set_condition`, `clear_condition`, `set_resource`): named conditions (Wounded, Hunted, Blessed…) and simple **progress clocks** (Blades-style N-segment tracks for looming threats/goals). Surface as chips in the scene banner + Journal. This turns dice and choices into consequences without building combat.
  2. **Bonds / relationships feeding the spotlight.** Party members are static — their opinion of the PC never moves. Add a per-member, adventure-scoped **Bond** value the narrator nudges on meaningful beats (a tool), injected into the spotlight block so a high/low bond changes who speaks and how. This is the already-envisioned Bond Gauge, delivered narratively before any combat exists.
  3. **Light progression surfaced in the Journal.** The PC/party don't visibly change across a long adventure. Add **milestones** (auto-detected or Chronicler-proposed) that can evolve a **Field Skill** or grant a titled trait — no XP math, just punctuated growth the player feels, shown on a Journal "growth" strip.
  4. **Goal-aware action suggestions.** The action panel generates one option per moral rule but ignores the active **tasks**. Feed the top task(s) into the suggester so at least one option advances a stated goal — tightening the loop between "what I can do" and "what I'm here to do."
  5. **Reactive world flags / reputation.** The Chronicler adds lore but the world holds no *state*. Add campaign/adventure **flags** (faction standing, world switches) the narrator can read and set, so earlier choices visibly change later scenes. Pairs with Pillars (always-injected rules) from R18.
  6. **Guided FTUE.** New players meet Play-vs-Edit, the action panel, and the Inspector with no orientation. Add the intended **first-time flow** (Editor dialogue → first Narrator beat) plus a dismissible one-time coach-mark pass. Ship 2-3 more genre **templates** (the R13 alternate openings + Pillars make templates far richer now).
  7. **Tone/genre & content knobs (ties into R21).** A per-campaign **tone/rating** dial (grim ↔ heroic, content limits) that seeds narrator + suggester guidance, so a world *feels* consistent without hand-editing instructions.

  Sequencing: 1 → 2 → 4 are the tightest ROI (all reuse the narrator tool-loop + prompt-builder + spotlight seams already in place); 3/5/6/7 are larger. Explicitly out of scope per the vision: the JRPG battle screen, AP economy, grid — don't scaffold for them. Note: several of these introduce **per-campaign rules** (bonds on/off, clocks, tone, stat/condition vocabularies) — see R21 for where those knobs should live.

---

[x] R21: Consider having a new Section for Campaign, for all the campaign-related settings that do not have anything to do with the Engine or its Appearance, but rather with all the knobs and levers of the adventure, e.g. defining stats, currency, party size, etc. Create a thought-out suggestion for this comprehensive change and addition.

  **Plan — a first-class "World Rules" (Campaign) settings surface.** R19 already introduced the three scopes as chips — **Global** (engine/LLM), **Device** (appearance), **Campaign** (this world). This task makes the CAMPAIGN scope its own home instead of being sprinkled through Config: today the campaign switcher lives under a "Campaign" section, but the actual *rules of the world* are scattered (party size is **Global** — `OpenRouterSettings.max_party_size`, already flagged in code "*move to per-adventure scope in a later phase*") or absent (no currency, no stats).

  **Proposed data model — a new campaign-scoped `CampaignRules` singleton** (its own table, `__table_args__ = CAMPAIGN`, so it rides along in campaign zips/exports like `NarratorConfig`), holding:
  - **Party** — `party_size` (moved off `OpenRouterSettings`; a one-time back-fill copies the old global value into each campaign, then the global field is retired).
  - **Currency** — `{ name, abbrev, symbol }` (e.g. "Gold / gp / ⛃"), optionally denominations. Wires into the existing `Currency` item type + inventory display and the narrator's item tools.
  - **Attributes / stats** — a definable list `[{ key, label, description }]` (e.g. STR/CON…), plus a mode: **flavor-only** (today's behaviour), **shown** (rendered read-only on the PC/member sheet), or **checked** (a later dice hook can use them). Nothing forces mechanics — it just lets a world *declare* its stat vocabulary, which the narrator/suggester see. These Attributes / stats are also added to the PC / Party Members (figure out where stats are saved to keep character cards usable across different campaigns, e.g. a separate json or stored in the campaign / adventure?)
  - **Dice** — move `NarratorConfig.dice_enabled` here and add the **DC map / difficulty labels** (easy/normal/hard/heroic) so a world can retune difficulty.
  - **Rarity tiers** — optional per-world rename/recolour of the item rarity ladder (c/u/r/e/l) already in the catalog.
  - **Tone / rating** — the per-campaign tone dial from R20 (grim ↔ heroic, content limits) that seeds narrator + suggester guidance.
  - (Action Option Rules + Spotlight already live on `NarratorConfig` and are per-campaign — surface them under this section too, even if stored where they are.)

  **UI.** A dedicated **"World Rules"** top section in Config (CAMPAIGN chip), grouping the above with the campaign switcher/export/import — clearly separated from **AI & Model / Agents** (Global) and **Appearance** (Device). Reuse the R19 sticky-nav + auto-save + scope-chip machinery, so it's consistent and drops in cleanly. Also expose these as **Editor tools** (`set_world_rules` / read) so Edit Mode can define currency, stats, party size, and tone conversationally — matching how the Scenario/lore are already edited.

  **Migration & phasing.** (1) Add `CampaignRules` + additive migration; back-fill `party_size` from the global setting; keep reading the old field until every campaign has a row. (2) Move `dice_enabled` over (dual-read during transition). (3) Wire currency into inventory/item display; wire the attribute list into the PC/member sheets (read-only first). (4) Templates ship rule presets (fantasy: Gold + STR/DEX/…; the empty template stays minimal). Endpoints mirror `/scenario` (`GET`/`PUT /campaign-rules`, singleton). This is the natural completion of R19's scope model and the home for R20's per-world knobs.

  **Implemented (phase 1).** New campaign-scoped **`CampaignRules`** singleton ([models.py](server/db/models.py)) — `party_size`, `currency_{name,abbrev,symbol}`, `attributes` (JSON `[{name,description}]`), `tone` — riding along in campaign zips. Additive migration creates the table + **back-fills `party_size` from the old app-global `max_party_size`** only for pre-existing campaigns (fresh ones get the table via create_all and their row from the template applier); the reader seam `_max_party_size` + the Chronicler's party-cap now read `CampaignRules` (global field kept only as the back-fill seed). REST `GET`/`PUT /campaign-rules` (singleton, mirrors `/scenario`); the ruleset is injected into the **narrator prompt** as a compact `WORLD RULES` block ([ai/rules.py](server/ai/rules.py) `compose_rules_block`, threaded via `_maybe_summarize_and_build` → `build_prompt(campaign_rules=…)`). Editor gets **`get_world_rules`/`set_world_rules`** tools (partial updates). New Config **"World Rules"** section (CAMPAIGN scope) with party size, currency, an attributes editor, and a tone box — auto-saving via a new `campaignRulesStore` (optimistic `patchLocal` + debounced PUT, R19-style); Max Party Size moved here off the Global "Party" subsection, and HomeView's `n / max` reads the campaign value. Fantasy template ships Gold/gp + Might/Agility/Wits/Spirit + a tone line. Verified: 73 pytest (3 new: `normalize_attributes`, `compose_rules_block`, REST round-trip incl. clamp), client tsc + vite build clean, and a Playwright runtime smoke against real data — the migration back-filled the live campaign's party size (1), the World Rules section renders under the jump-nav, and edits PUT `/campaign-rules` and persist. **Deferred** (noted): moving `dice_enabled` here; wiring currency/attributes onto the item + PC/member sheets; a "checked" attribute mode for a future dice hook.

---
[x] New Campaign should be a guided **Story Style** builder instead of a template dropdown: the player picks Genre, Tone, Style of Writing, Verbosity, Content Limit, Perspective, Structure (each with an "etc."/custom option), plus a freeform custom-instructions field, and those choices build out the core of the Narrator's instructions. Options + their prompt snippets must live in an easily editable JSON so new options can be added without a rebuild.

  Done (commit e997103): The **option catalog lives in an editable JSON** ([server/ai/style_catalog.json](server/ai/style_catalog.json)) — fields, options, labels/hints, and each option's prompt snippet — read at runtime and cached by file mtime (`WAYWARD_STYLE_CATALOG` override), so editing a snippet or adding an option takes effect on the next request with no rebuild. [server/ai/style.py](server/ai/style.py) is a thin loader + composer: `compose_style_block` folds the player's selections into a `STORY STYLE` system block injected right after the narrator instructions in [prompt_builder.py](server/ai/prompt_builder.py) (same shape as the WORLD RULES block), plus `normalize_style_fields`, `to_wire`/`from_wire` (camelCase API ↔ snake_case storage), `migrate_legacy_tone`, and `options_payload` (promptless picker payload). Selections persist as a new campaign-scoped **`NarratorConfig.style_fields`** JSON column (additive migration); a known option id uses its snippet, any other value is free-text (`"Label: <text>"`). The **New Campaign modal is now the builder** (name + guided dropdowns with per-field "Custom…" + a "Start from the Fantasy demo world" checkbox) — the template dropdown, `GET /campaigns/templates`, and `list_templates()` are gone; `apply_template(name, style_fields=)` still supplies universal defaults, `empty`/`fantasy` kept as internal seeds. Choices **stay editable** in a new Config → **Story Style** section (`storyStyleStore`, optimistic + debounced save) via `GET`/`PUT /campaign-style` (+ `/campaign-style/options`), and the Editor gets **`get_story_style`/`set_story_style`** partial-update tools ("make it darker", "write more like Pratchett", "third person"). **Tone reconciliation:** Story Style now owns tone — a one-time lazy migration folds a legacy `CampaignRules.tone` into `style_fields.tone` on first read and the World Rules Tone box is removed (pointer note left); tone injects via WORLD RULES *or* STORY STYLE, never both. `DEFAULT_NARRATOR_INSTRUCTIONS` trimmed of its hardcoded second-person/length clauses (now the Perspective/Verbosity options; builder preselects second person / standard / adult). Back-compat: pre-builder campaigns have `style_fields=NULL` → no block → identical prompts. Verified: 86 pytest (new `test_story_style.py` pure + integration, plus prompt-builder block-ordering tests), client tsc + vite build + vitest clean, and an end-to-end run confirming builder selections persist (camelCase→snake) and compose into the narrator prompt with real snippets + custom text + trailing custom instructions.

---
[x] Consolidate the Narrator's instructions surface: remove the old free-text Narrator Instructions box from Config → World, rename the "Story Style" section to "Narrator Instructions", ensure the combined instructions always begin with a clear role definition, and move the core instructions (everything that isn't a style option or custom instructions) into a single easily-editable JSON — no rebuild.

  Done (commit f6c6d6a): The Narrator's always-on text now lives in [server/ai/style_catalog.json](server/ai/style_catalog.json) under a top-level `narrator` object — `core_instructions` (role + core behavior), `tool_guidance`, `dice_guidance`, and `formatting_guide` — read at runtime (mtime cache, `WAYWARD_STYLE_CATALOG` override) so they're editable with no rebuild; [server/ai/style.py](server/ai/style.py) exposes `core_instructions()`/`tool_guidance()`/`dice_guidance()`/`formatting_guide()` with in-code fallbacks so the role/conventions are never lost if the JSON is broken. [prompt_builder.py](server/ai/prompt_builder.py) now injects `core_instructions()` **first** (prompt always opens with a clear role definition), then a per-campaign override (`NarratorConfig.instructions`) only when it's a genuine custom value — a stored built-in default is matched against `_LEGACY_DEFAULT_INSTRUCTIONS` and skipped so the JSON core isn't duplicated — then the `STORY STYLE` block. [narrator_agent.py](server/ai/narrator_agent.py) pulls its tool/dice/formatting guidance from `style` instead of module constants. `apply_template` seeds `instructions=""` (the JSON core is the universal default now; the Editor's `set_narrator_instructions` still writes the optional override layer). Client: removed the base Narrator Instructions textarea from Config → World, renamed the "Story Style" Config section and the New Campaign modal heading to **Narrator Instructions**, and dropped the now-unused `instructions` wiring. Verified: 90 pytest (4 new — core/guide fallbacks, live JSON edit, role-first ordering, legacy-default dedup), client tsc + vite build + vitest clean, and an end-to-end run confirming new campaigns seed an empty base and an edit to `core_instructions` in the JSON is reflected live (no restart) and leads the prompt.
---
[ ] (from the architecture audit) Give OpenRouter errors a taxonomy and stop blindly retrying safety blocks. Today both provider errors and safety-filter blocks surface as a single `RuntimeError` ([openrouter.py](server/ai/openrouter.py)), and `auto_retry_count` retries all of them identically ([agent_turn_with_retry](server/ai/openrouter.py) catches every exception). Distinguish rate-limit (429) / auth / billing (402) / model-not-found (404) / network / safety, and surface specific guidance to the UI ("try again in a minute" vs "out of credits" vs "this model is gone"). Do NOT silently auto-retry a safety block — regenerating the same prompt N times burns credits for the same block; show it to the player instead (optionally offering edit-and-resend).

[ ] (from the architecture audit) Reverse partial tool effects when an agentic narrator turn fails terminally. In `run_narrator_agent` ([narrator_agent.py](server/ai/narrator_agent.py)) tools commit to the DB during the loop and their deltas are recorded on the `ChatMessage` only after a successful narration. A normal per-model-call retry is already safe (it re-sends unmutated messages and never re-runs committed tools), but if a round exhausts all retries and the turn raises, tools committed in earlier rounds are orphaned — in the DB, attached to no message, so `_reverse_message_effects` can never undo them. On terminal turn failure, reverse the accumulated inventory/equipment/scene deltas so a failed turn leaves no half-applied world state. (Deliberately do NOT wrap the whole loop in one open transaction — the model must see committed state between rounds, e.g. grant then equip, and holding a write transaction across network round-trips would worsen the cross-file WAL lock contention.)

[ ] (from the architecture audit) Calibrate context-token estimation from real usage. Prompt trimming in [build_prompt](server/ai/prompt_builder.py) uses a chars/4 heuristic, which drifts badly on JSON/code/non-English text and can either trim context the model could have kept or overflow the model's window. The real `prompt_tokens`/`completion_tokens` already stored per assistant message (`ChatMessage`, used by the context meter) should feed back a per-model chars-per-token ratio so trimming and the `max_tokens` clamp track actual token counts rather than a fixed 4.0.
...
Done (13 commits, 0ecf6a0..68c40a6, merged a5e5989): a new **Species** lorebook category replaces the old flat **Monsters** category. Each entry now has 8 structured fields — Overview, Physical Appearance, Biology & Reproduction, Culture & Behavior, Danger & Combat Notes, Typical Gear, Archetypes & Variants, Name Examples — composed into the entry's `content` at write time via `compose_species_content()` ([server/ai/species.py](server/ai/species.py)), mirroring the Scenario tab's structured-fields pattern; the lore-injection pipeline needed zero changes. A one-time, idempotent DB migration recategorizes existing `monsters` rows to `species` and carries their old freeform text into Overview. Wired through everywhere a lore entry can be written: the generic `/lore` API, the Chronicler's `create_lore`/`update_lore` proposals (with reversal support), and the Editor's equivalent tools — `content` is always kept in sync with the structured fields, never hand-typed for a species entry. Client: the Monsters tab is renamed Species everywhere (LorePanel, CategoryIcon, SettingsPanel, PartyInspector), and the Lore Inspector's edit form now renders the 8 labeled fields instead of one textarea when editing a Species entry. Verified: 102 server tests passing (1 pre-existing unrelated failure), 20 client vitest passing, clean client build.

The larger, related **Character/Party-Member unification** design (making Lore Characters share Party Members' rich schema, campaign-scoped character storage, non-destructive promote/demote) was brainstormed and spec'd alongside this — see [docs/superpowers/specs/2026-07-18-character-unification-design.md](docs/superpowers/specs/2026-07-18-character-unification-design.md) — but is a bigger migration, not yet implemented.

---
[x] Bug (Android build 0.1.63, 100% repro): Narrator Instructions' Story Style selections don't stick — restarting the APK resets them.

Root cause: `App.tsx`'s boot effect (the one that fires once on app load and fetches every store — narrator config, scenario, world rules, etc.) never called `useStoryStyleStore.getState().fetchFields()`. `fetchFields` existed and was correctly wired into `adventuresStore.reloadAll()` (used for campaign/adventure *switches*), but a fresh app load never goes through that path — it only runs the App.tsx boot effect. So on every launch the Story Style store started from `EMPTY_FIELDS` (all blank) regardless of what was actually saved server-side — Config → Narrator Instructions *looked* fully reset. Worse, it wasn't just cosmetic: `StoryStyleSection`'s Save button PUTs the *entire* current `fields` object (all 8 keys, always present as strings), so pressing Save after a blank load round-tripped empty strings for every field you hadn't touched that session — and the server's `normalize_style_fields` drops blank values, so those previously-saved selections were actually deleted from `NarratorConfig.style_fields` on save, not just hidden. (Confirmed the selections *do* reach the Narrator correctly via `compose_style_block` in `prompt_builder.py` once persisted — the bug was purely the missing initial fetch, not the injection wiring.)

Fix: added `fetchStoryStyle` (→ `useStoryStyleStore.fetchFields`) to the App.tsx boot effect, alongside the other per-campaign config fetches (scenario, world rules, etc.), so Story Style selections are loaded from the server on every app launch — Android restart included — not just on a campaign/adventure switch. Verified: client `tsc -b` + `vite build` + `vitest` all clean.

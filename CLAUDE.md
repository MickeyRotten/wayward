...
Instead of a template dropdown, the **New Campaign modal is a guided builder**: the player picks **Genre / Tone / Style of Writing / Verbosity / Content Limit / Perspective / Structure** (each with a "Custom…" free-text option) plus freeform **custom instructions**, and those choices compose the core of the Narrator's voice. Choices **stay editable** after creation (Config → **Story Style**) and via Editor tools.

- **Editable catalog JSON.** The option catalog — fields, options, labels/hints, and each option's **prompt snippet** — lives in [`server/ai/style_catalog.json`](server/ai/style_catalog.json), read at runtime and cached by file mtime (`WAYWARD_STYLE_CATALOG` override). Adding/tuning an option = editing this JSON; **no rebuild**. [`server/ai/style.py`](server/ai/style.py) is a thin loader + composer: `compose_style_block(fields)` folds the player's selections into a `STORY STYLE` system block, `normalize_style_fields`, `to_wire`/`from_wire` (camelCase API ↔ snake_case storage), `migrate_legacy_tone`, `options_payload` (promptless picker payload).
- **Storage & injection.** Selections persist as `NarratorConfig.style_fields` (JSON, campaign scope, additive migration). A value matching an option id uses that option's snippet; any other non-empty string is free-text (`"Label: <text>"`). `build_prompt` appends the composed block right after the narrator instructions (mirrors the WORLD RULES block). Empty/NULL → no block, so pre-builder campaigns get identical prompts.
- **API & UI.** `GET /campaign-style/options` (catalog for the picker), `GET`/`PUT /campaign-style` (singleton, partial PUT, mirrors `/scenario`). Client: `storyStyleStore` + `Settings/CampaignBuilder.tsx` (`StyleFieldsEditor` shared by the modal and the Config section). Editor tools `get_story_style`/`set_story_style` (partial per-field update, like `set_scenario`).
- **Tone reconciliation.** Story Style **owns tone** going forward. A one-time lazy migration folds a legacy `CampaignRules.tone` into `style_fields.tone` on first `GET /campaign-style` and clears the old field; `compose_rules_block` keeps its tone branch for un-migrated campaigns, so tone injects via WORLD RULES *or* STORY STYLE, never both. `DEFAULT_NARRATOR_INSTRUCTIONS` no longer hardcodes perspective/length (now the Perspective/Verbosity options; the modal preselects second person / standard / adult).

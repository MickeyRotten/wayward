...
- **Fixed actions** (no LLM cost, always shown) sit **above the composer**, under the "OR DO SOMETHING ELSE:" header (not in the in-chat panel): Continue ("I wait and let the scene unfold."), Look Around, Talk to Party, Rest, and Use an Item (inline inventory popover via `ItemCard`; picking sends `"I use the <item>."`).

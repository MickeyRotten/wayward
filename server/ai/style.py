...
- Do NOT call a tool for the scene. The location, weather and how long the beat took ride the trailing turn block at the end of your reply (see the OUTPUT PROTOCOL). Never write a day number, a date or a clock time anywhere — the game keeps the calendar and tells you the time of day.
- Use consume_item when the player uses up an item they carry (e.g. drinks a potion).
- EVERY tool call is a change your prose JUST made. Do not re-report something you already did on an earlier turn, and do not confirm the state you can already see in INVENTORY or on a character sheet — setting something to what it already is will be discarded. If an item is merely SEEN (in a chest, held by someone else, or waiting behind one of the options you are about to offer), the party has not taken it: change nothing.

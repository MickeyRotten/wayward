...
- *Write:* `grant_item`, `remove_item`, `consume_item` (replaces the old deterministic item-use keyword scan), `equip`, `unequip`. **`set_scene` is deliberately NOT offered** — scene state rides the trailing `<<<TURN>>>` block instead of buying a round-trip (see "The Turn Block and the Clock"); the handler survives only for the legacy text protocol. (History summarisation is deterministic and server-side: when the prompt crosses the threshold, compression runs as a **post-turn background pass** — `_summarize_in_background` in server/api/chat.py — never blocking the player's turn.)
...
**Model support & narration mode (`tool_mode`).** How the narrator mutates state is chosen by `OpenRouterSettings.tool_mode` (Config → Agents & Tools → **Tool Mode**), resolved per-turn by `_resolve_narration_mode` (`server/api/chat.py`): **auto** (native tool loop when the selected model advertises `supportsTools`, else the text-block path), **native** (force the tool loop), **text** (force the hardened `<<<ACTIONS>>>` protocol — the reliable path for strong *narrative* models that call tools poorly), **off** (pure prose, no state mutation). `tool_mode` supersedes the legacy `use_tools` boolean (kept only as the migration seed: `True`→`auto`, `False`→`text`). The text path retains `parse_action_block`/`execute_actions`/`ACTION_INSTRUCTION`; `parse_action_block` is deliberately tolerant (loose markers, missing END marker, brace-matched JSON salvage, always strips the block from prose so malformed JSON can't leak). `max_tool_rounds` also lives on `OpenRouterSettings`.

...
- **Chronicler** — a passive post-turn world-builder that proposes lore/task/member additions, and runs the **Steward** alongside it every turn, which keeps item possession (pickups/equips/losses) accurate on a separate, tool-capable model — independent of the narrator's own (possibly unreliable) tool calling.
- **Edit Mode (the Editor)** — a foreground, conversational world-builder; full CRUD over the world.
- **Lorebook** (world/characters/items/species/spells, with keyword injection), including a structured **Species** category — 8-field creature/people templates (Overview/Physical Appearance/Biology & Reproduction/Culture & Behavior/Danger & Combat Notes/Typical Gear/Archetypes & Variants/Name Examples), covering both sapient peoples and monsters/creatures in one category (see "The Lorebook — Species & Creature Templates"), **tasks** (a flat to-do list; the successor to quests+objectives), **objectives** (overarching, direction-setting goals that steer the narrator) and a player **wishlist** (soft steers the narrator keeps in mind) — both alongside tasks in the Tasks panel, **inventory + a unified item system** (items are lore entries; owned copies are non-stacking `ItemInstance`s — see "Data Models"), **equip/unequip + Drop** from the item Inspector, a **crop/zoom portrait editor**, a structured **Scenario** tab (Setting/History/Species/Geography/Technology & Magic/Other — see "The Scenario"), and a **server-owned in-game clock** (the narrator names a duration; the game keeps the calendar — see "The Turn Block and the Clock").
- **The action panel** — the primary text-adventure interaction: numbered AI-generated choice options (a configurable count, all shaped by one shared instruction to stay varied; scripted options on the opening beat) plus the fixed actions, rendered in-chat under the latest beat, with the freeform input demoted below an "Or do something else:" header (see "Action Suggestions").
...
- **Attributes (STR/CON/…) remain narrative flavor only** — there is no dice mechanic, attribute-modifier math, HP, or combat resolution.
...
- **Items are lorebook entries** (`LorebookEntry`, `cat == "items"`, carrying type/slot/rarity/maxStack/uses) — the shared *catalog* (campaign-scoped). No separate item table. The lorebook also holds world/characters/species/spells, with keyword-based injection (`LorebookConfig`).
...
- **Characters are portable files, not DB rows — ONE PNG per character**, SillyTavern-card-compatible (`server/data/characters/<id>.png`; replaced the old folder-of-4-files layout). Storage is raw PNG `tEXt` chunks, no Pillow ([`server/db/png_card.py`](server/db/png_card.py)): standard `chara`(V2)/`ccv3`(V3) chunks carry a composed, cross-tool-readable `description` etc; `data.extensions.wayward` carries the real data (`name`, the block tree, schema version); `chara-ext-asset_:{path}` chunks (the V3 asset mechanism) hold every binary blob — `full.<ext>` (portrait), `crop.<ext>` (chat crop), `voice.<ext>` (TTS sample), `img/<id>.<ext>` (extra reference images). Reads are **asset-first** (`portrait_bytes`/`crop_bytes`/`voice_bytes` in [`server/db/characters.py`](server/db/characters.py)) — the outer PNG's own pixels are only a best-effort preview for non-Wayward tools (swapped in when an upload already happens to be a PNG), so no image-format conversion is ever needed for Wayward's own reads.
  - **The block tree** ([`server/ai/character_blocks.py`](server/ai/character_blocks.py)) replaced the fixed `basicInfo` fields — a TavernAI-style toggleable/orderable list: `text` (freeform, `{{name}}` resolves to the character's own name), `folder` (groups children; OFF disables the whole subtree), `equipment` (**virtual** — rendered live from the current `PartyBinding`/catalog at prompt time, never stored — see `render_equipment_block`-style composition in `prompt_builder.py`), `image` (points at an embedded asset). Every character is seeded with **Open Tag/Close Tag** blocks (`<Name>...</Name>`, editable/removable) wrapping its sheet, so multiple sheets concatenated in one prompt (PC + party) stay clearly delimited to the model. `compose_blocks()` flattens enabled blocks to text (auto-labeling non-Description/non-tag blocks, e.g. `"Personality: ..."`); `image_block_paths()` lists enabled image blocks for a future vision-attachment pass.
  - **Legacy compatibility projection.** A lot of call sites (Editor tools, the Chronicler, `seed.py`/`templates.py`) still talk in the old flat field shape (`species`/`sex`/`apparentAge`/`description`/`personality`/`instinct`/`strengths`/`other`). `RuntimeCharacter.basic_info` (see below) is a **read-only projection** of the block tree back onto that shape (`project_legacy_basic_info`), used by those Python-level callers only. `BasicInfoSchema` in `schemas.py` mirrors the same field names as the **wire** shape for that read-only projection and the legacy combined `PUT /player-character`/`PUT /party-members/{id}` routes, but real editing no longer goes through it (see below).
  - **Block-tree UI.** `CharacterSheetEditor`/`PartyMemberEditor` (`client/src/components/CharacterSheet/BlockTreeEditor.tsx`, shared by both) render the actual block tree — a flat-or-folder toggle/reorder list (checkbox to enable/disable, ▲▼ to reorder, inline rename, an expandable textarea per `text` block, a live read-only note for the virtual `equipment` block, delete-with-inline-confirm, "+ ADD BLOCK"/"+ ADD INSIDE" to create `text`/`folder`/`equipment` blocks — folders nest one level deep, no roles/depth/merge-groups). A read-only `BlockTreeView` renders the same data as plain labeled paragraphs in Play mode. Persisted via dedicated routes — `PUT /player-character/blocks` / `PUT /party-members/{id}/blocks` (`{blocks, name?}`, `CharacterBlocksUpdate` in `schemas.py`) — rather than the old combined PUTs: those round-tripped through `BasicInfoSchema`'s legacy field set, which doesn't carry `strengths`/`other`/`instinct`/etc, so every identity save was silently wiping those blocks back to empty. Equipment saves got their own dedicated `PUT .../equipment` routes for the same reason (a combined save would carry the same stale-schema identity payload alongside it). The old combined routes still exist (harmless, unused by the client) since `BasicInfoSchema` itself was corrected to the real field names. **Still open:** no Editor tool for direct block add/toggle/reorder (the Editor still writes the old flat fields, which fold into blocks via `upsert_legacy_field`); no upload route/UI for `image` blocks (storage support exists — see `set_image_asset` below — but nothing calls it, and there's no vision-attachment wiring).
  - **Import** ([`import_png`](server/db/characters.py)) accepts either a native Wayward card (round-trips its block tree from `extensions.wayward`) or a **plain SillyTavern V2/V3 card PNG** — each non-empty standard field (description/personality/scenario/mes_example/system_prompt/post_history_instructions/creator_notes/first_mes) becomes its own block, wrapped in the Open/Close tags (`blocks_from_sillytavern`). `character_book` (an embedded per-card lorebook in the ST spec) is **not** mapped yet — real Lorebook-entry conversion is future work. Export-to-plain-SillyTavern (stripping the `wayward` extension) is not built.
  - Per-adventure state — worn **equipment**, `in_party`, `last_spoke_turn`, and `role` (`pc`|`member`) — lives in an adventure-scoped **`PartyBinding`** row referencing the character id. [`server/db/party.py`](server/db/party.py) joins the two into `RuntimeCharacter` composites (`load_pc`/`load_party`) with binding writers (`set_equipment`/`set_in_party`/`set_last_spoke`); the app reads `.basic_info` (the legacy projection), `.blocks` (the real data), `.equipment`/`.in_party`/`.last_spoke_turn`/`.id`, and a computed `.field_skill` property (Strengths-block content, split back into `{name, description}` for old readers). A `/characters` REST API lists/imports (`.zip` or a bare card `.png`)/duplicates/deletes cards, serves/uploads portraits, and zips a card for sharing (the zip now just wraps the one PNG). `migrate_characters_to_files` (in [`database.py`](server/db/database.py)) converts legacy `PlayerCharacter`/`PartyMember` rows into cards+bindings on load; a separate one-time **folder→single-PNG** migration (`_migrate_legacy_folder`, on first `read_character`) upgrades any pre-rebuild character folder in place. Campaign zip export/import bundles the referenced character PNGs (falls back to the old per-folder shape for zips exported before this rebuild).
...
- **Species** (`LorebookEntry`, `cat == "species"`) is edited as 8 structured fields (`species_fields` JSON column, same pattern as `scenario_fields`), composed into `content` via `compose_species_content()` ([`server/ai/species.py`](server/ai/species.py)) — see "The Lorebook — Species & Creature Templates". Replaces the old flat `monsters` category (merged; covers both sapient peoples and monsters/creatures in one category).
- **NarratorConfig** (campaign-scoped) holds `instructions`, `action_instruction`, `spotlight_rule`, `post_history_instructions`, `first_message`, `first_message_options` (scripted opening choices for the primary opening, JSON), `first_message_alternates` (JSON list of `{message, options}` — alternate openings, each with its OWN scripted options; at turn 0 the chat swipe arrows cycle `[first_message, *alternates]` showing each opening's own options, and the chosen one is anchored per-adventure to `StorySummary.opening_message`, which then overrides the primary message in display + prompt; authored as per-opening cards under the Scenario tab's "Opening Messages" section — `normalize_openings` in ai/scenario.py coerces the legacy bare-string shape), `planner_instructions`, `action_suggestions_enabled` (default on), `action_suggestions_instructions`, `action_suggestions_mode` (`separate`|`inline`, **default `inline`** — see "Action Suggestions"), `action_suggestions_count` (how many options to generate, 1-6, default 4 — the options are all shaped by the single shared `action_suggestions_instructions`), `action_option_rules` (JSON; **legacy** per-slot rules — superseded by the shared instruction + count, kept only to seed the count on migration and for wire back-compat) (each text field falls back to a built-in default when blank), and `style_fields` (JSON; the Campaign Builder's **Story Style** selections — see "Story Style (the Campaign Builder)").
- **OpenRouterSettings** (app-scoped) holds the api key (never returned to client), model/sampling params, `max_tokens_response`, `max_context_tokens`, `max_party_size`, plus agent settings `use_tools` (legacy — superseded by `tool_mode`, kept only as its migration seed), `tool_mode` (`auto`|`native`|`text`|`off` — the narrator's state-mutation path; see "The Narrator Agent Loop" → Model support & narration mode), `max_tool_rounds`, `auto_retry_count` (auto-regenerate on an error/safety block; default 2, 0-5 — see "The Narrator Agent Loop"), `reasoning_effort` (''=provider default | low/medium/high | **off** (explicitly disables reasoning, freeing the whole budget for narration) — sent as OpenRouter's `reasoning.effort`/`reasoning.enabled` on narrator calls only, never to strict providers; reasoning deltas render live as "REASONING · ~N WORDS", and an all-reasoning empty reply surfaces a clear error), `worldbuilding_mode`, `worldbuilding_interval` (Chronicler cadence in player turns, 1-10, default 2), `worldbuilding_model_id`, `action_suggestions_model_id`, `planner_model_id` (optional separate **Editor** model, blank → main model), and TTS settings `tts_enabled`/`tts_autoplay`. Real **usage accounting** is requested on every OpenRouter stream and stored per assistant message (`ChatMessage.prompt_tokens`/`completion_tokens`/`gen_cost`, summed across tool rounds; shown as a dim per-message meta label, and the context meter prefers it over the chars/4 estimate). It also carries the **provider selector** `llm_provider` (`openrouter`|`nvidia_nim`|`custom`, default `openrouter`) with per-provider write-only creds+model — `nim_api_key`/`nim_model_id`, `custom_base_url`/`custom_api_key`/`custom_model_id` (`api_key`/`model_id` are OpenRouter's) — resolved via `provider_endpoint` (see "OpenRouter Integration (+ multi-provider)"). (The old carry-slot limit was removed — inventory is unbounded.)
- **ChatMessage** (adventure-scoped) carries `role`, `content`, `turn_number`, `variant`, `speaker`, `mode` (`narrator`|`planner`), scene state (`location`, `weather` narrator-declared; `time_of_day` is a derived phase word and `day`/`scene_minutes` are server-owned — see "The Turn Block and the Clock"), `spotlight_reason`, `applied_inventory_deltas`/`applied_equipment_changes` (for swipe/regenerate/delete reversal), and `editor_actions` (planner messages: the `{name, result}` tool actions the Editor took — see "Edit Mode").
...
- Also: `PartyBinding` (adventure-scoped character↔adventure state; identity is files — see above), `Task` (flat to-do list — replaced `Quest`+`QuestObjective`; legacy tables kept only for the one-time `migrate_quests_to_tasks` back-fill; a task's `notes` are injected into the narrator prompt and editable by the Chronicler/Editor), `Objective` (adventure-scoped overarching, direction-setting goals — bigger than a Task; `text`/`status`/`detail` (the "stakes" free-text); injected into the narrator prompt as `OVERARCHING OBJECTIVES`; `/objectives` CRUD + Editor tools), `Wish` (adventure-scoped player **Wishlist** — `text`/`priority` 0-3; player-authored only, never touched by the agents; injected as a soft `PLAYER WISHLIST` steer; `/wishes` CRUD), `ItemInstance` (+ legacy `InventoryStack`), `StorySummary` (auto-maintained; surfaced read-only via `GET /journal` — see "Journal"), `WorldbuildingProposal` (Chronicler), `ChatEvent` (adventure-scoped persistent in-chat toasts: Chronicler notices tethered to their turn, untethered player item actions — kinds `chronicler`|`item`; see "Chat Rendering & Narration Formatting"), `AppState` (active campaign/adventure pointer). Legacy `PlayerCharacter`/`PartyMember` tables remain only for `migrate_characters_to_files`.
- **`GameContext`** ([`server/api/chat.py`](server/api/chat.py)) is what one narration turn reads, loaded once: settings/narrator/PC/active party/**benched**/messages/summary/catalog/tasks/objectives/wishes/lore/lore-config plus the derived **`scene`** (latest-wins location/day/minutes/weather) and **`inventory_lines`**. It replaced a twelve-element tuple unpacked identically in four places and threaded onward as six keyword arguments.
- **SQLite conventions:** every attached DB runs `journal_mode=WAL` + `synchronous=NORMAL` + `busy_timeout` (set in `_attach`, [`database.py`](server/db/database.py)) so the narrator's writes and the concurrent post-turn Chronicler/suggester reads never contend. Hot filter columns carry `index=True` (new files) plus `CREATE INDEX IF NOT EXISTS` back-fills in `_run_scope_migrations`. Per-turn history loads are **bounded**: the chat turn reads a `_HISTORY_WINDOW` (**160**, ~80 turns — comfortably past the largest prompt budget; 500 was ten times what any context can hold and every turn paid for the read) newest-message window; the Chronicler/suggester make targeted per-turn queries — never load a whole adventure.
...
3. `max_tool_rounds` (default **4**, configurable) caps the loop; the final round drops `tools` to force narration. Each round is a full round-trip that re-sends the *entire* prompt, so this is the most expensive dial in the app — withdrawing `set_scene` removed the single most common round, and 4 is comfortably more than a turn needs.
4. A **duplicate safe write inside one turn** (same tool, same canonical args) is suppressed and reported back as "already done this turn" — models re-request a write they made two rounds ago, and a duplicate grant is a real second grant.

**Tools** (handlers in [`server/ai/narrator_actions.py`](server/ai/narrator_actions.py)):
- *Write:* `grant_item`, `remove_item`, `consume_item` (replaces the old deterministic item-use keyword scan), `equip`, `unequip` — collectively `ACTION_HANDLERS`, the **one canonical vocabulary** shared with the text protocol below (`_HANDLERS` in `narrator_agent.py` is `{**ACTION_HANDLERS, ...the read tools, set_scene}`; `_SAFE_WRITE_TOOLS` is `frozenset(ACTION_HANDLERS)`). **`set_scene` is deliberately NOT offered as a native tool** — scene state rides the trailing `<<<TURN>>>` block instead of buying a round-trip (see "The Turn Block and the Clock"); the handler survives only for very old data. (History summarisation is deterministic and server-side: when the prompt crosses the threshold, compression runs as a **post-turn background pass** — `_summarize_in_background` in server/api/chat.py — never blocking the player's turn.)
- *Read:* `lookup_item`, `search_items`, `list_inventory`, `get_character` — let the model validate before acting (e.g. confirm an item exists and its slot before `equip`). Native-only: a one-shot text-protocol block has no round-trip to react to a read result.
...
**Model support & narration mode (`tool_mode`).** How the narrator mutates state is chosen by `OpenRouterSettings.tool_mode` (Config → Agents & Tools → **Tool Mode**), resolved per-turn by `_resolve_narration_mode` (`server/api/chat.py`): **auto** (native tool loop when the selected model advertises `supportsTools`, else the text-block path), **native** (force the tool loop), **text** (force the hardened `<<<ACTIONS>>>` protocol — the reliable path for strong *narrative* models that call tools poorly), **off** (pure prose, no state mutation). `tool_mode` supersedes the legacy `use_tools` boolean (kept only as the migration seed: `True`→`auto`, `False`→`text`). `max_tool_rounds` also lives on `OpenRouterSettings`.

**One protocol, native as an accelerator.** The text path (`parse_action_block`/`execute_actions`/`ACTION_INSTRUCTION` in `narrator_actions.py`) used to be a separately-maintained, drifted vocabulary (a documented `removeItems` key with no implementation at all; no `consume_item` equivalent; its own hand-rolled character/slot resolution instead of the native tools' no-op guarantees). It's now unified: the trailing `<<<ACTIONS>>>` block's canonical shape is `{"actions": [{"tool": "grant_item"|"remove_item"|"consume_item"|"equip"|"unequip", ...same arg names as TOOL_SCHEMAS}, ...]}`, and `execute_actions` dispatches every entry through the exact same `ACTION_HANDLERS` the native loop uses — so the text protocol gets the native path's no-op guarantees (a redundant equip/grant is silently skipped) and `_failure_note` failure reporting (`toolFailures` on `done`) for free, instead of re-implementing a weaker version of both. `parse_action_block` is deliberately tolerant regardless (loose markers, missing END marker, brace-matched JSON salvage, always strips the block from prose so malformed JSON can't leak); `execute_actions` also still reads the pre-unification `addItems`/`removeItems`/`equip`/`unequip` top-level-array shape (folded into the same canonical entries) for backward compatibility, though it's no longer what `ACTION_INSTRUCTION` teaches. Native tool-calling is simply a faster, validated *transport* for this one vocabulary — not a second one. (Still separate, deliberately: the two streaming drivers in `chat.py`, since one is a multi-round loop and the other a single completion + post-hoc parse; and item actions still cost native models a real tool round — moving them onto the trailing block too remains a distinct, not-yet-done follow-up.)

**Preamble handling & tool-arg repair (DeepSeek-family reliability).** Some strong narrative models narrate the whole beat and only *then* append a state-write tool call. Rather than discarding that prose and regenerating (the "write→delete→rewrite" artifact), the loop **accepts a substantial narrated beat as final** when every requested tool is a non-prose-shaping safe write (`_SAFE_WRITE_TOOLS` = grant/remove/consume/equip/unequip) — it runs the tools and stops. The read tools still force a clean re-narration (their result must be free to shape the prose). When content *is* genuine tool preamble and gets discarded, it's dropped from the transcript too (`content=None`) so the model re-narrates cleanly instead of continuing text the player can no longer see. Malformed tool-call argument JSON is reported back as a tool result for the model to resend (`_parse_args` → `None`), instead of silently running with empty args. Inline `<think>…</think>` chain-of-thought (from providers without a reasoning field) is stripped from the content stream by `_ThinkStripper` (`openrouter.py`) and routed to the reasoning channel — leading block only, so ordinary prose is untouched.
...
**Cadence.** It runs every `OpenRouterSettings.worldbuilding_interval` player turns (1-10, **default 2**; Config → Agents & Tools → Chronicler), covering the whole span since it last ran (`chronicler_span`, deterministic in the turn number so it needs no bookkeeping and a swipe lands on the same decision). Halving the runs halves a whole extra generation per turn *and* reads better: judging "is this genuinely new?" from a single beat is what made it re-propose the entry it wrote two turns ago. An interval of 1 is the old every-turn behaviour exactly; `POST /worldbuild/run {force:true}` bypasses the cadence.
...
### The Steward (item possession)

A **second agent pass inside the same module** (`_run_steward_pass` in [`server/ai/worldbuilder.py`](server/ai/worldbuilder.py)) — not a new subsystem. It exists because the narrator's own state-mutation path (`tool_mode`: `native`/`text`) is unreliable on older/weaker narration models: `native` needs real tool support, and the `text` (`<<<ACTIONS>>>`) fallback still asks a possibly non-tool-capable model to emit precise structured deltas in the same breath as prose. The Steward instead reads the **finished** narration afterward, on a separate, genuinely tool-capable model, and performs item mutations via validated tool calls — narrator concerns (prose) and state concerns (deltas) stay split, the same reasoning that keeps `set_scene` off the narrator's native tool schema.

- **Runs every turn**, independent of the Chronicler's `worldbuilding_interval` cadence — it always looks at just the newest turn's narration, gated only by a cheap deterministic pre-filter (`_worth_stewarding` — a possession-verb regex: picks up/grabs/takes/loots/hands/gives/buys/equips/wears/wields/drops/loses/sells/consumes/…) so most turns (no possession change) cost nothing.
- **Tools**: `take_item` (the party now physically has something — mints a new lorebook `items` entry when it's genuinely new, given a description/type/slot; grants an existing one by exact name; optional `equipOnCharacter` grants-and-equips in one call), `equip_item` (equip something already held), `drop_item` (lost/consumed/given away/sold, or — with `unequipOnly` — just unequipped back into the pack). Guidance (`STEWARD_GUIDANCE`) mirrors the Chronicler's no-duplicates/no-restatement rules, scoped to possessions.
- **Always auto-applies**, regardless of `worldbuilding_mode` (confirmation vs. auto) — a player who can't pick up or equip items has no game, so item proposals (`WorldbuildingProposal.kind == "item"`) go straight to `accepted`/`failed`, never `pending`. `worldbuilding_mode == "disabled"` is the one setting that turns the Steward off too (it's the same LLM-call gate as the Chronicler's).
- **No-op guarantees carry over**: a `take_item` with no *explicit* count for something already stowed is a restatement and records no delta (same rule as the narrator's own `grant_item` tool); an `equip_item` for what's already worn in that exact slot is a no-op; reuses `inv_ops.grant_items`/`equip_instance`/`remove_items` (never reimplements grant/equip logic) so these guarantees come for free.
- **Reversal**: accepted item proposals carry their `_invDeltas`/`_equipChanges` in the payload (same shape the narrator's own tools record) plus a `_createdNew` flag; `reverse_chronicler_effects` (shared with the Chronicler's own lore/task reversal) inverts them on swipe/regenerate/delete — a brand-new catalog entry is deleted, but granting-then-reversing an item that already existed leaves the catalog alone.
- **Toasts**: an accepted item proposal gets a tethered `ChatEvent(kind="item", tethered=True)`, rendered the same way the Chronicler's own auto-apply toasts are (removed together with the turn on reversal).
- **Recommended pairing**: set the narrator's `tool_mode` to **`off`** (pure prose; scene/options still ride the `<<<TURN>>>` block, which is independent of `tool_mode`) and let the Steward — on a separate, tool-capable `worldbuilding_model_id` — own every item mutation instead. Party-member creation stays out of the Steward's scope entirely; it's still the Chronicler's `create_member` tool, with its `_is_named_character` guard unchanged.
...
- **Numbered choice options** (1.-6., clickable; number keys submit too) under the latest beat. After each narrator beat these are AI-generated by the one-shot agent in [`server/ai/action_suggester.py`](server/ai/action_suggester.py) — **`action_suggestions_count` options** (1-6, default 4), all shaped by a **single shared instruction** (`action_suggestions_instructions`, blank → built-in default) which tells the model to keep the *set* varied (spread across cautious/bold, kind/ruthless, practical/curious) rather than following per-slot rules — phrased as the PC's impulses: the PC's **Personality & Drive** (basicInfo fields, edited on the PC sheet) lead the suggester's context and both mode guidances. The count is edited in Config → Agents & Tools. (The legacy per-slot `action_option_rules` were dropped as the shaping mechanism — situations vary too much for universal per-slot rules; the column is retained only for count-seeding on migration and wire back-compat.) On the **opening beat** (no player turns yet) the options are the **scripted** `NarratorConfig.first_message_options`, authored next to the First Message in its Scenario-tab Inspector (templates may supply them — fantasy.json does); the suggester never runs at turn 0. A **↻ REROLL** button sits with the options (re-POSTs `/action-suggestions/run`; the route accepts `turn: null` → latest turn, so reroll works even after a refresh; hidden on the opening beat).
...
Mechanics: **two generation modes** (`NarratorConfig.action_suggestions_mode`, Config → Agents & Tools): `inline` (**default**) has the options ride the narrator's trailing `<<<TURN>>>` block — no second call, no extra wait, tied to the main model (see "The Turn Block…"); `separate` runs the Chronicler-style one-shot tool call (`suggest_actions`, `minItems == maxItems == action_suggestions_count`) after the turn — its own small LLM call, optionally on a cheaper model; (guidance injected by the stream drivers via `build_turn_block_guidance`, parsed + stripped by `parse_turn_block` before persisting, options ride the `done` SSE event). When an inline block yields no options, the client falls back to the separate call automatically; **reroll and the self-healing fetch always use the separate call**. No DB persistence either way, transient per turn; custom `action_suggestions_instructions` replace the preamble, and the requested option count is appended. Gated by `NarratorConfig.action_suggestions_enabled` (**per-campaign**, default **on** for new campaigns — an extra small LLM call per turn; existing campaigns keep their stored setting) with an optional model override `OpenRouterSettings.action_suggestions_model_id` (blank → main model). Fire-and-forget from `chatStore` after each narrator turn (`POST /action-suggestions/run`); every option/fixed action just calls the existing `sendTurn` with its text — no special submission path. When suggestions are disabled or empty the panel still shows the fixed actions.
...
The framing premise for the whole world, edited as **6 structured fields** in a dedicated **Scenario tab** in the Lore panel (shown first, before World/Characters/Items/Species/Spells): **Setting**, **History (Brief)**, **Species**, **Geography**, **Technology & Magic**, **Other**.
...
## The Lorebook — Species & Creature Templates

A **Species** entry (`LorebookEntry`, `cat == "species"`) is a reusable creature/people template — **one category covers both sapient peoples and monsters/creatures** (replaces the old flat `monsters` category). Edited as **8 structured fields**: **Overview**, **Physical Appearance**, **Biology & Reproduction**, **Culture & Behavior**, **Danger & Combat Notes** (narrative flavor only — no combat system exists yet), **Typical Gear**, **Archetypes & Variants**, **Name Examples**.

- Same pattern as the Scenario: the fields are the source of truth, composed into the entry's freeform `content` via `compose_species_content()` ([`server/ai/species.py`](server/ai/species.py)) — skipping empty fields — so the lore-injection pipeline needed zero changes. Unlike Scenario, Species is **not a singleton** — an entry per species/creature, stored via the generic `/lore` API (`speciesFields` rides alongside `title`/`content`/`keywords` on create/update, merged partially via `merge_species_fields`).
- **Storage**: a `species_fields` JSON column directly on `LorebookEntry` (only meaningful when `cat == "species"`).
- **Legacy migration**: `migrate_species_lore()` ([`database.py`](server/db/database.py)) is a one-time, idempotent DB migration that recategorizes existing `cat == "monsters"` rows to `"species"`, carries their old freeform `content` into `Overview` (`migrate_legacy_species_fields`), and renames the `"monsters"` key to `"species"` in the campaign's `LorebookConfig` injection settings.
- **Chronicler**: recorded on first real appearance (encountering/fighting/learning about a species is itself worth recording — unlike Characters, no player-engagement gate). **Editor**: `create_lore`/`update_lore` accept the same `speciesFields` object, partial-update on edit.
- Content is **always** recomposed from `species_fields` whenever a write touches a `cat == "species"` row (create or update) — a hand-typed `content` is never persisted for a Species entry, so the two can't drift.

---

## Edit Mode (the Editor)

A **foreground** world-builder. Engine-style framing: **Play mode = Narration** (runtime), **Edit Mode = building the game**. Toggle it with the Unity-style **Play button on the left of the chat location banner** (lit gold while playing). When on, the chat's primary agent becomes the **Editor** ([`server/ai/planner.py`](server/ai/planner.py) — internal names still say "planner"/`planner_instructions`/`/planner/...`) with full CRUD over lore (all categories incl. dedicated `create_item`/`update_item` with type/slot/rarity + `equip`/`unequip`), tasks, party members, the PC, the **Scenario**, and the **Story Style** (including the narrator's freeform custom instructions via `set_story_style`'s `customInstructions` field — the Editor does **not** edit the core Narrator instructions/role). You converse with it and it creates/edits many things per turn, then replies conversationally. It runs on the main model unless `OpenRouterSettings.planner_model_id` overrides it (Config → AI & Model → **Editor**).
...
- After an Editor turn the client refreshes lore/tasks/party/items/narrator **plus scenario/story-style/world-rules** panels (`refreshWorldPanels` in `chatStore`), so structured-config edits (`set_scenario`/`set_story_style`/`set_world_rules`) apply live in the UI instead of going stale until reload; the Chronicler does **not** run for Editor turns.
...
- **Templates:** creating a campaign runs through a **New Campaign modal** — now the **Story Style builder** (see below), not a template dropdown. Templates remain plain JSON files in [`server/templates/`](server/templates/) (`empty.json`, `fantasy.json`) used as **internal seeds**: [`server/db/templates.py`](server/db/templates.py) `apply_template(name, style_fields=None)` reads one and populates the fresh campaign/adventure DBs — narrator config, scenario, lore, keyed catalog items, PC, party, and inventory (written as catalog-id equipment + `InventoryStack`, then converted via `migrate_to_item_instances`). New campaigns always use `empty`; the modal's **"Start from the Fantasy demo world"** checkbox picks `fantasy`. **Universal defaults:** the applier always stores non-empty Spotlight Rule / Editor Instructions (built-in defaults unless the template overrides). The Narrator's **core** instructions are no longer seeded per-campaign — they live in `style_catalog.json` (`narrator.core_instructions`, see "Story Style"), so `NarratorConfig.instructions` starts empty (an optional Editor-tool override layer).

---

## Story Style (the Campaign Builder)

Instead of a template dropdown, the **New Campaign modal is a guided builder**: the player picks **Genre / Tone / Style of Writing / Verbosity / Content Limit / Perspective / Structure** (each with a "Custom…" free-text option) plus freeform **custom instructions**, and those choices compose the Narrator's voice. Choices **stay editable** after creation (Config → **Narrator Instructions**, formerly "Story Style") and via Editor tools. This section is the narrator-instructions surface: the old free-text base-instructions textarea was removed from Config.

- **Editable JSON — options *and* core.** [`server/ai/style_catalog.json`](server/ai/style_catalog.json) is read at runtime and cached by file mtime (`WAYWARD_STYLE_CATALOG` override); editing it is **no rebuild**. It holds two things: the Story Style **`fields`** catalog (options, labels/hints, and each option's **prompt snippet**), and a top-level **`narrator`** object with the always-on instruction blocks — **`core_instructions`** (the Narrator role + core behavior), **`tool_guidance`**, and **`formatting_guide`**. [`server/ai/style.py`](server/ai/style.py) is the loader + composer: `compose_style_block(fields)` folds selections into a `STORY STYLE` block; `core_instructions()`/`tool_guidance()`/`formatting_guide()` return the JSON blocks (with in-code fallbacks so the role/conventions are never lost); plus `normalize_style_fields`, `to_wire`/`from_wire`, `migrate_legacy_tone`, `options_payload`.
- **Prompt assembly.** `build_prompt` injects `style.core_instructions()` **first**, so the prompt always begins with a clear role definition; then an optional per-campaign override (`NarratorConfig.instructions`) only when it's a genuine custom value (a stored built-in default — matched against `_LEGACY_DEFAULT_INSTRUCTIONS` — is skipped so the JSON core isn't duplicated); then the `STORY STYLE` block. The agentic loop ([`narrator_agent.py`](server/ai/narrator_agent.py)) pulls `tool_guidance`/`formatting_guide` from `style` too. New campaigns seed `instructions=""` (the JSON core is the universal default now); `NarratorConfig.instructions` is now a **legacy override layer** (still injected when non-empty, but the Editor no longer writes it — the `set_narrator_instructions`/`get_narrator_instructions` tools were removed). The Editor instead edits the freeform **custom instructions** via `set_story_style` (the `custom_instructions` style field), so it can shape narration guidance without touching the core Narrator role.
- **Storage & injection.** Style selections persist as `NarratorConfig.style_fields` (JSON, campaign scope, additive migration). A value matching an option id uses that option's snippet; any other non-empty string is free-text (`"Label: <text>"`). Empty/NULL → no STORY STYLE block, so pre-builder campaigns get identical prompts.
- **API & UI.** `GET /campaign-style/options` (catalog for the picker), `GET`/`PUT /campaign-style` (singleton, partial PUT, mirrors `/scenario`). Client: `storyStyleStore` + `Settings/CampaignBuilder.tsx` (`StyleFieldsEditor` shared by the modal and the Config **Narrator Instructions** section). Editor tools `get_story_style`/`set_story_style` (partial per-field update, like `set_scenario`).
- **Tone reconciliation.** Story Style **owns tone** going forward. A one-time lazy migration folds a legacy `CampaignRules.tone` into `style_fields.tone` on first `GET /campaign-style` and clears the old field; `compose_rules_block` keeps its tone branch for un-migrated campaigns, so tone injects via WORLD RULES *or* STORY STYLE, never both. The core instructions no longer hardcode perspective/length (now the Perspective/Verbosity options; the modal preselects second person / standard / adult).
...
- **Signing:** every build (debug and release) is signed with the same persistent key (`signingConfigs` in `build.gradle.kts`), so any build installs over any other without uninstalling and user data survives updates. The keystore itself is **not committed** (`android/signing/` is gitignored) — CI restores it from the `WAYWARD_KEYSTORE_B64` repo secret (base64-encoded keystore file) and reads `WAYWARD_KEYSTORE_PASS`/`WAYWARD_KEY_ALIAS`/`WAYWARD_KEY_PASS` from secrets too; a build fails loudly if any is missing rather than falling back to a hardcoded password. For local builds, keep your own untracked copy of the keystore at `android/signing/wayward-release.keystore` and export those same three env vars yourself. `versionCode` = 1 + the Actions run number. Don't regenerate the keystore — a new key breaks in-place updates for existing installs.

---

## Prompt Assembly

One isolated function — [`build_prompt`](server/ai/prompt_builder.py) — assembles every narration call, in **five tiers** running oldest-and-general → newest-and-specific:

```
TIER 1 · STANDING CONTEXT   (one stable, cacheable prefix)
  core narrator instructions · legacy per-campaign override · STORY STYLE ·
  WORLD RULES · PC sheet (+ equipment) · PARTY SHEETS (who they are)

TIER 2 · TURN CONTEXT       (keyword-gated; skipped whole on a quiet turn)
  matched lore (top) · PARTY SPOTLIGHT · absent-member note

TIER 3 · HISTORY            (trimmed to budget; First Message prepended)

TIER 3b · STORY SO FAR      (the stretch the window has already dropped)

TIER 4 · STATE OF PLAY      (one authority line over all of it)
  CURRENT SCENE (location · day · phase · weather) · PARTY roll call ·
  INVENTORY · OVERARCHING OBJECTIVES · ACTIVE TASKS · PLAYER WISHLIST

TIER 5 · THIS TURN, THEN THE ASK
  lore (before_input) · post-history instructions · OUTPUT PROTOCOL
  (the <<<TURN>>> block) · the player's message · lore (bottom)
```

**Two rules hold the shape together**, and both are easy to break by adding "one more block":

- **Every fact is stated once.** A model shown the same fact twice re-states it, and a re-statement costs a state write, a `ChatEvent` and a line of transcript. Equipment is on the sheets; who is *present* is in the roll call; nothing is in both.
- **Anything the history can contradict is stated AFTER the history.** That is the whole reason tier 4 exists. The roster, the pack and the task list are exactly what the last twenty beats misremember, and the later statement is the one the model believes — which is why a companion who left kept getting voiced.

The reordering is also a **performance** property: tier 1 is byte-identical between turns of an adventure, so it stays a stable prompt prefix a provider can cache. The old order put the roster and the task list — which change constantly — ahead of everything, invalidating the prefix every single turn.

**The active-party roll call** ([`server/ai/roster.py`](server/ai/roster.py) `compose_roll_call`) is re-read from the bindings every turn: who is here (`n/partySize`), who is benched and explicitly "NOT in this scene", and — the case that matters most — **it is emitted even when the party is empty** ("the player is ALONE"), because the empty case is precisely where the history drifts.

**One scan window.** `CONTEXT_TURNS` (prompt_builder) is shared by lore matching and the spotlight's recent-context scan, so "mentioned" means one thing across every gate. (`LorebookConfig.scan_depth` still overrides it per campaign.)
...
## The Turn Block and the Clock

Two deterministic seams that take state back from the narrator. The through-line: **the narrator writes prose and names what changed; the server decides every number.**

### The trailing `<<<TURN>>>` block — [`server/ai/turn_block.py`](server/ai/turn_block.py)

Scene state used to cost a whole model round-trip: the model called `set_scene` with four strings, the loop executed it, appended a tool result, and called the model *again* to write the prose. A round-trip is the most expensive thing a turn can spend, and it was buying four strings the model could append to the beat it had just written. So `set_scene` **left the native tool schema** (the handler survives for the legacy text protocol) and scene now rides one trailing block alongside the player's next options:

```
<narration prose>

<<<TURN>>>{"location":"Damp Cellar","weather":"cold and still","duration":"scene","options":["I search the racks.","I climb back up."]}
```

- **Emit canonical, read lenient.** `build_turn_block_guidance` documents exactly one shape and ships a **worked example built from the enabled fields** — a named field is an *invitation*, so a field nothing reads is never mentioned. `parse_turn_block` accepts marker variants (`<<TURN>>`, `[TURN]`, any case, the legacy `<<<OPTIONS>>>`) **only when a JSON object follows**, so the word "turn" in prose can never truncate a beat; accepts a **dropped marker** (the last brace-balanced object whose keys intersect the contract — without this the raw JSON was rendered into the reading pane as narration); repairs raw newlines inside string literals; and normalises the four shapes of `options` models reach for (array, newline string, `{action:…}` rows, a numbered map). The block is **always** stripped, even when its JSON is malformed.
- `StreamingWindow` truncates the display at `<<<` so the block never flashes mid-stream.
- The block supersedes the inline `<<<OPTIONS>>>` mode; `NarratorConfig.action_suggestions_mode` now defaults to **`inline`**, so the common turn spends one model call instead of two. `separate` remains the reroll and self-heal path.

### The clock — [`server/ai/clock.py`](server/ai/clock.py)

`day` was a narrator-written integer that nothing validated: it could freeze, jump, or run backwards, and one confused beat desynced the calendar permanently. It is **out of the protocol entirely**. The narrator emits a `duration` label off a fixed ladder (`moment · brief · scene · hour · hours · halfday · day · night`) and the server owns every number:

- An unrecognised label falls to the smallest step, so a garbage value is *unrepresentable* rather than clamped.
- A block with **no** duration still advances time, so an unparseable turn can never freeze the world.
- `night` **anchors** to the next 07:00 rather than adding a span, so sleeping always lands on a morning; an anchor further out than `MAX_ANCHOR_REACH` degrades to a half-day (a nap taken at eight in the morning is a nap).
- Time reaches the player and the model as a **phase word** only (`phase_of`, eight bands) — never a clock face. `ChatMessage.scene_minutes` (additive migration) carries the internal minute, so swipe/regenerate/delete reversal restores the clock for free: dropping the message drops its time. Swipe and regenerate advance from where the **turn** started, so re-telling a beat never ages the world twice.
- The client mirrors the vocabulary once in [`client/src/lib/clock.ts`](client/src/lib/clock.ts) (`daylightToken` / `sunShape` / `phaseEmoji`), read by the backdrop matcher, the scene header and the banner icon — three places that each carried their own five-value time map.
- `coerce_scene` (narrator_actions) also guards the legacy text path: `day` is accepted only when it moves forward, and **`simplify_location`** keeps the last segment of a narrator-stapled compound ("Boars Head Tavern - Damp Cellar" → "Damp Cellar") for the ` - ` / ` — ` / ` / ` / `: ` joiners only — never a comma ("Rodstroke, Mesmeria" nests the other way) and never a bare hyphen ("Half-Moon Inn"). Wayward feels a drifting location twice over, because it also drives the backdrop matcher.

### No-op writes

**An op that sets X to what X already is is not an op.** A narrator shown the pack every turn and asked what changed answers with the *state* instead of the change — the reflex that turns one rusty key into seven. So: `grant_item` with **no explicit count** for something already stowed does nothing (an explicit count is always honoured — "two more torches" is a real event); `equip` of what is already worn in that slot records no equipment change (or reversal would replay a bogus one forever); `unequip` of an empty slot and `remove_item` for something not held are **no-ops, not failures** (reporting a failure invites the model to try again). The agent loop also suppresses a **duplicate safe write inside one turn**, keyed on `(name, canonical args)`.
...
**Mobile layout (<1024px):** `useIsMobile` ([`client/src/lib/useIsMobile.ts`](client/src/lib/useIsMobile.ts)) swaps `AppShell` for [`MobileShell`](client/src/components/Layout/MobileShell.tsx) — one full-screen view at a time over a bottom tab bar ([`MobileNav`](client/src/components/Layout/MobileNav.tsx); primary tabs + a "More" sheet), with the Inspector as a full-screen slide-over that opens on selection and closes via its Back header. Same stores/components either way — only the shell differs. **The Android hardware Back button closes the overlay instead of quitting the app** ([`useBackHandler`](client/src/lib/useBackHandler.ts)): while an overlay is open it pushes ONE spare history entry and treats the resulting `popstate` as "close this", popping the entry back off on unmount so the stack never grows. Nested overlays each claim their own entry, so Back unwinds them in order (the Inspector and the nav's More sheet both use it).
...
│   ├── lib/     api.ts, clock.ts (the phase vocabulary, mirrored from the
│   │            server), useBackHandler.ts (Android hardware Back closes the
│   │            overlay, not the app), location.ts (scene-banner derivation), narration.ts
...
│   │   ├── prompt_builder.py   build_prompt — the five tiers (see Prompt Assembly)
│   │   ├── roster.py           the active-party roll call (stated after the history)
│   │   ├── turn_block.py       the trailing <<<TURN>>> block (emit canonical, read lenient)
│   │   ├── clock.py            the duration ladder → server-owned day/minutes/phase
...
│   │   ├── worldbuilder.py     the Chronicler (post-turn world-building) + the Steward (item possession, every turn)
...
│   │   ├── species.py          Species field composition (8 fields → derived content, monsters merge)
...
- **Server:** `python -m pytest server/tests` (deps: `pip install -r server/requirements-dev.txt`). Pure-function tests for the deterministic seams (lore injector, prompt builder **incl. tier order and the stable tier-1 prefix**, spotlight, the clock/turn-block/roll-call seams in `test_clock_turn_block.py`, action options, provider resolution, summariser helpers) plus TestClient integration tests (story export, narrator item tools + reversal + no-op writes, the turn block end-to-end through `/chat/turn` with a stubbed provider in `test_turn_block_e2e.py`, Chronicler `_prev` restore, background summary, the Steward's take/equip/drop + reversal in `test_worldbuilder_steward.py`, the unified `execute_actions` vocabulary/dedup/no-op/failure-reporting in `test_execute_actions.py`, and its `<<<ACTIONS>>>` block end-to-end through `/chat/turn` in `test_actions_block_e2e.py`). **`WAYWARD_DATA_DIR`** overrides the data root (`server/db/database.py`); conftest sets it to a temp dir before any server import so tests never touch a real `server/data` — keep it that way when adding tests. Any test that creates + loads its own adventure (rather than reusing `test_app_integration.py`'s `boot_adventure_id`) must restore whichever adventure was active before it in a fixture teardown — `boot_adventure_id` is session-scoped and captures "whatever is active" on its first resolution, so an earlier-sorting test file that leaves a different adventure active silently corrupts it for the rest of the session (see the `adventure`/`isolated_adventure` fixtures in the test files above for the pattern).
- **Client:** `npm test` in `client/` (vitest; dedicated `vitest.config.ts`, node env) — unit tests for the pure libs (`narration.ts`, `weather.ts`, `backdrops.ts`, `sortEntries.ts`, `clock.ts`) as `src/**/*.test.ts` siblings.

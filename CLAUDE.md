...
- **Characters are portable files, not DB rows — ONE PNG per character**, SillyTavern-card-compatible (`server/data/characters/<id>.png`; replaced the old folder-of-4-files layout). Storage is raw PNG `tEXt` chunks, no Pillow ([`server/db/png_card.py`](server/db/png_card.py)): standard `chara`(V2)/`ccv3`(V3) chunks carry a composed, cross-tool-readable `description` etc; `data.extensions.wayward` carries the real data (`name`, the block tree, schema version); `chara-ext-asset_:{path}` chunks (the V3 asset mechanism) hold every binary blob — `full.<ext>` (portrait), `crop.<ext>` (chat crop), `voice.<ext>` (TTS sample), `img/<id>.<ext>` (extra reference images). Reads are **asset-first** (`portrait_bytes`/`crop_bytes`/`voice_bytes` in [`server/db/characters.py`](server/db/characters.py)) — the outer PNG's own pixels are only a best-effort preview for non-Wayward tools (swapped in when an upload already happens to be a PNG), so no image-format conversion is ever needed for Wayward's own reads.
  ...
  - **Legacy compatibility projection.** A lot of call sites (Editor tools, the Chronicler, `seed.py`/`templates.py`) still talk in the old flat field shape (`species`/`sex`/`apparentAge`/`description`/`personality`/`instinct`/`strengths`/`other`). `RuntimeCharacter.basic_info` (see below) is a **read-only projection** of the block tree back onto that shape (`project_legacy_basic_info`); writes through `update_identity`/`update_member_identity` normalize old-er-shaped input (`migrate_basic_info` — gender→sex, age→apparentAge, a separate fieldSkill→strengths — the wire shape `BasicInfoSchema` in `schemas.py` still sends) and upsert into the matching-named block (`upsert_legacy_field`), creating it if missing. **Not yet done:** no block-tree UI (CharacterSheet/PartyMember still render the old flat form against this projection) and no Editor tool for direct block add/toggle/reorder — both flagged follow-ups, see TODO.md.

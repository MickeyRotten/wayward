...
- **ChatMessage** (adventure-scoped) carries `role`, `content`, `turn_number`, `variant`, `speaker`, `mode` (`narrator`|`planner`), scene state (`location`, `weather` narrator-declared; `time_of_day` is a derived phase word and `day`/`scene_minutes` are server-owned — see "The Turn Block, the Clock, and the Dice"), `spotlight_reason`, `applied_inventory_deltas`/`applied_equipment_changes` (for swipe/regenerate/delete reversal), and `editor_actions` (planner messages: the `{name, result}` tool actions the Editor took — see "Edit Mode").
...
- *Write:* `grant_item`, `remove_item`, `consume_item` (replaces the old deterministic item-use keyword scan), `equip`, `unequip`. **`set_scene` is deliberately NOT offered** — scene state rides the trailing `<<<TURN>>>` block instead of buying a round-trip (see "The Turn Block, the Clock, and the Dice"); the handler survives only for the legacy text protocol. (History summarisation is deterministic and server-side: when the prompt crosses the threshold, compression runs as a **post-turn background pass** — `_summarize_in_background` in server/api/chat.py — never blocking the player's turn.)
...
- *Dice:* `skill_check(characterName, skill, difficulty)` — offered only when `NarratorConfig.dice_enabled`. **The server rolls the d20** ([`server/ai/dice.py`](server/ai/dice.py); DC map easy 8 / normal 12 / hard 16 / heroic 19, nat 1/20 = crits) and returns roll/DC/outcome, so the model narrates a result it was *given*. The roll is **seeded on `(turn, variant, actor, skill, difficulty)`**, not random — see "The Turn Block, the Clock, and the Dice". Each roll writes a **tethered `ChatEvent` (`kind='dice'`)** rendered as a gold/red dice chip; being tethered, it vanishes with the turn on swipe/regenerate/delete, and the retelling reproduces the same roll (a swipe rolls fresh). Agentic path only.
...
## The Turn Block, the Clock, and the Dice

Three deterministic seams that take state back from the narrator. The through-line: **the narrator writes prose and names what changed; the server decides every number.**
...
### The dice — [`server/ai/dice.py`](server/ai/dice.py)

`skill_check` was `random.randint`, which put a save-scum hole behind the regenerate button: re-roll until the answer is the one you wanted and no failure ever has to be lived with. Rolls are now **seeded on `(turn, variant, actor, skill, difficulty)`** — a regenerate re-*tells* the same result, a swipe (a deliberate request for a different take) rolls fresh, and choosing another action earns a new roll on its own.

Two seeding traps, both regression-tested, both the reason this is thirty lines and not three: FNV-1a's bit 0 is the XOR of the input bytes' low bits and `h % n` shares its parity for even `n`, so a raw hash fed to the modulo makes a die's parity a function of the seed's characters; and hashing `seed|i` for extra dice locks them into fixed relative parities (**2d6 could never roll 7**). One hash, avalanched through a murmur3 finalizer, extra dice counting off that base.
...
│   │   ├── dice.py             seeded skill-check rolls (no save-scumming)
